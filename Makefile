...
# GRAND-ALL: every achievable offload at once — the four base offloads with
# Level-B DEEP recon (sim-ro-query) + field-pages (skip bytecheck). SIM_AIR_VKEY
# is not built (infeasible as a clean stub — the verify is AIR-object-based and
# half of build_epoch_airs is the proof-bound register commitment). Measure with
# LAMBDA_VM_SIM_CONSTRAINT_PRELOAD=1.
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-grandall.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-deeprecon-bench,--features "$(SIM27_BASE_NOINPLACE) sim-ro-query sim-poly-eval sim-pow sim-fold-chain sim-verify-path-batch sim-constraint-eval sim-field-pages sim-domain-points sim-register-commit")
