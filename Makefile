...
# BATCHED GRAND-ALL (sim/36 Phase 2): the batched grand-levelA-v2-fextapi-extmul
# base plus all nine mid-level stub offloads. RO goes Level-B via sim-ro-query,
# which SUBSUMES the base sim-ro-ecalls (sim-ro-query pulls it in) — the batched-
# incompatible sim-ro-inplace is never involved. Built from the BATCHED binary
# (grand-levelA-v2-bench) so the guest keeps the batched verify path; the required
# sim-ro-ecalls is satisfied transitively. EXECUTE-ONLY, NEVER prove (sim ecalls
# drive no chip, and fext x batched has a proving incompat). Measure with
# LAMBDA_VM_SIM_CONSTRAINT_PRELOAD=1 (constraint-eval is a real host offload).
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-levelA-v2-fextapi-extmul-grandall.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-levelA-v2-bench,--features "continuation blowup4 sim-hash-ecalls sim-ro-query sim-path-ecall bump-alloc inv-hint fext-accel attest-commitment-id fext-base-mul fext-inv fext-ext-mul sim-poly-eval sim-pow sim-fold-chain sim-verify-path-batch sim-constraint-eval sim-field-pages sim-domain-points sim-register-commit")

.PHONY: compile-recursion-grand-levelA-v2-fextapi-extmul-grandall-elf
compile-recursion-grand-levelA-v2-fextapi-extmul-grandall-elf: prepare-sysroot \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-levelA-v2-fextapi-extmul-grandall.elf
...
# =============================================================================
# sim/27 MID-LEVEL accelerator measurement stubs. Each ELF is the sim/22 endgame
# baseline (grand-v2-fextapi-extmul, 53,483,890 cycles) plus ONE sim/27 stub
# feature so its cycle delta is measured independently; `-simallmid` turns all of
# them on together. MEASUREMENT-ONLY — measure with `cli execute --cycles`, NEVER
# prove (the sim-* ecalls drive no chip). The shared feature prefix is the exact
# extmul baseline set above.
# =============================================================================
SIM27_BASE_FEATURES := continuation blowup4 sim-hash-ecalls sim-ro-inplace sim-path-ecall bump-alloc inv-hint fext-accel attest-commitment-id fext-base-mul fext-inv fext-ext-mul

$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simpolyeval.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_BASE_FEATURES) sim-poly-eval")

$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simpow.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_BASE_FEATURES) sim-pow")

$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simfoldchain.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_BASE_FEATURES) sim-fold-chain")

# sim-constraint-eval is a real host offload (v2): the CLI preloads the captured
# constraint programs, so measure with LAMBDA_VM_SIM_CONSTRAINT_PRELOAD=1.
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simconstraint.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_BASE_FEATURES) sim-constraint-eval")

# The four-stub all-on (R9 baseline gate, 25,737,059 cycles with the preload).
SIM27_ALLMID_FEATURES := $(SIM27_BASE_FEATURES) sim-poly-eval sim-pow sim-fold-chain sim-constraint-eval
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simallmid.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_ALLMID_FEATURES)")

# =============================================================================
# sim/27 R9 stubs: each SOLO on top of the four-stub all-on base, plus grand-all
# (all seven). SIM_FIELD_PAGES skips the rkyv bytecheck; SIM_DEEP_RECON offloads
# the per-query DEEP reconstruction; SIM_AIR_VKEY serves AIR metadata as vkey.
# Measure the constraint-carrying ones with LAMBDA_VM_SIM_CONSTRAINT_PRELOAD=1.
# =============================================================================
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-allmid-fieldpages.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-bench,--features "$(SIM27_ALLMID_FEATURES) sim-field-pages")

# SIM_DEEP_RECON is realized by the EXISTING Level-B whole-per-query DEEP ecall
# (`sim-ro-query`, REDUCED_OPENING_QUERY): it computes (deep_eval, deep_eval_sym)
# = the FRI fold's p0 inputs, offloading the whole reconstruct-pair (the per-row
# RO rows + the in-guest trace-term FEXT accumulation the base's Level-A leaves).
# Level A/B are mutually exclusive, so this base drops sim-ro-inplace for
# sim-ro-query. Measure with LAMBDA_VM_SIM_CONSTRAINT_PRELOAD=1.
SIM27_BASE_NOINPLACE := continuation blowup4 sim-hash-ecalls sim-path-ecall bump-alloc inv-hint fext-accel attest-commitment-id fext-base-mul fext-inv fext-ext-mul
$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-allmid-deeprecon.elf: FORCE | prepare-sysroot $(RECURSION_ARTIFACTS_DIR)
	$(call build_guest_elf,$(RECURSION_GUESTS_DIR)/recursion,recursion-cont-blowup4-grand-v2-deeprecon-bench,--features "$(SIM27_BASE_NOINPLACE) sim-ro-query sim-poly-eval sim-pow sim-fold-chain sim-constraint-eval")

.PHONY: compile-recursion-sim27-midlevel-elfs
compile-recursion-sim27-midlevel-elfs: prepare-sysroot \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simpolyeval.elf \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simpow.elf \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simfoldchain.elf \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simconstraint.elf \
	$(RECURSION_ARTIFACTS_DIR)/recursion-cont-blowup4-grand-v2-fextapi-extmul-simallmid.elf
