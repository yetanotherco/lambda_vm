...
pub fn keccak_leaves_base(
    ...
    crate::nvtx_range!("gpu:keccak_leaves_base");
    ...
    crate::gpu_span!(&stream, "gpu:keccak_leaves_base");
    let cols_dev = {
        crate::nvtx_range!("h2d");
        stream.clone_htod(&columns[..total])?
    };
    ...
    };
    {
        ...
        crate::timing::timed_sync(&stream, "gpu:keccak_leaves_base")?;
    }
    Ok(out)
}
...
pub fn keccak_leaves_ext3(
    ...
    crate::nvtx_range!("gpu:keccak_leaves_ext3");
    assert!(num_rows.is_power_of_two());
    assert!(rows_per_leaf == 1 || rows_per_leaf == 2);
    assert!(
        num_rows >= rows_per_leaf,
    ...
    let be = backend()?;
    let stream = be.next_stream();
    crate::gpu_span!(&stream, "gpu:keccak_leaves_ext3");
    let cols_dev = {
        crate::nvtx_range!("h2d");
        stream.clone_htod(&columns[..total])?
    };
    ...
    {
        crate::nvtx_range!("sync");
        crate::timing::timed_sync(&stream, "gpu:keccak_leaves_ext3")?;
    }
    Ok(out)
}
...
pub fn build_merkle_tree_on_device(hashed_leaves: &[u8]) -> Result<Vec<u8>> {
    crate::nvtx_range!("gpu:build_merkle_tree");
    ...
    let be = backend()?;
    let stream = be.next_stream();
    crate::gpu_span!(&stream, "gpu:build_merkle_tree");
    ...
    let out = {
        crate::nvtx_range!("d2h");
        stream.clone_dtoh(&nodes_dev)?
    };
    {
        crate::nvtx_range!("sync");
        crate::timing::timed_sync(&stream, "gpu:build_merkle_tree")?;
    }
    Ok(out)
}
...
pub fn gather_merkle_paths_dev(
    ...
    crate::nvtx_range!("gpu:gather_merkle_paths");
    crate::gpu_span!(stream, "gpu:gather_merkle_paths");
...
pub fn build_comp_poly_tree_from_evals_ext3_keep(
    ...
    crate::nvtx_range!("gpu:comp_poly_tree_keep");
    let (nodes_dev, num_leaves, stream) = build_comp_poly_tree_nodes_dev(parts_interleaved)?;
    crate::gpu_span!(&stream, "gpu:comp_poly_tree_keep");
    let mut root = [0u8; 32];
    stream.memcpy_dtoh(&nodes_dev.slice(0..32), &mut root)?;
    crate::timing::timed_sync(&stream, "gpu:comp_poly_tree_keep")?;
...
pub fn build_fri_layer_tree_from_evals_ext3(evals: &[u64]) -> Result<Vec<u8>> {
    crate::nvtx_range!("gpu:fri_layer_tree");
    assert!(
    ...
    let be = backend()?;
    let stream = be.next_stream();
    crate::gpu_span!(&stream, "gpu:fri_layer_tree");
    ...
    crate::timing::timed_sync(&stream, "gpu:fri_layer_tree")?;
    Ok(out)
}
