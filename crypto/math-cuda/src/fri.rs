...
impl FriCommitState {
    ...
    pub fn fold_and_commit_layer(
        ...
        let cfg = LaunchConfig {
            grid_dim: (((n_out as u32) + 128 - 1) / 128, 1, 1),
            block_dim: (128, 1, 1),
            shared_mem_bytes: 0,
        };
        ...
        {
            let mut leaves_view = nodes_dev
                .slice_mut(leaves_offset_bytes..leaves_offset_bytes + num_leaves * 32);
        ...
        // Update inv_twiddles for the next layer: `new[j] = old[2j]²` for
        // j in 0..n_out/2. (If n_out == 1, skip — no next fold.)
        let tw_next = n_out / 2;
        if tw_next > 0 {
            let grid = ((tw_next as u32) + 128 - 1) / 128;
            let cfg = LaunchConfig {
                grid_dim: (grid, 1, 1),
                block_dim: (128, 1, 1),
                shared_mem_bytes: 0,
