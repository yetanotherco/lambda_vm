...
/// Transpose a `rows × cols` matrix on device: src (row-major) → dst (column-major).
/// dst must be pre-allocated with rows * cols elements.
pub(crate) fn launch_matrix_transpose(
    stream: &CudaStream,
    be: &Backend,
    src: &CudaSlice<u64>,
    ...
    let cfg = LaunchConfig {
        grid_dim: (cols.div_ceil(32), rows.div_ceil(32), 1),
        block_dim: (32, 32, 1),
        shared_mem_bytes: 0,
    };
    unsafe {
        stream
            .launch_builder(&be.matrix_transpose)
            .arg(src)
            .arg(dst)
            .arg(&rows)
            .arg(&cols)
...
/// Transpose + scatter: row-major `rows × cols` → column-major with out_stride.
/// Element src[r * cols + c] lands at dst[c * out_stride + r].
/// Requires out_stride >= rows. Used to scatter the row-major trace directly
/// into the LDE buffer (col_stride = lde_size) without an intermediate temp.
fn launch_matrix_transpose_strided(
    stream: &CudaStream,
    be: &Backend,
    src: &CudaSlice<u64>,
    dst: &mut CudaSlice<u64>,
...
/// Row-major variant of [`coset_lde_batch_base_into_with_merkle_tree_keep`].
///
/// Input `row_major` is a flat `n * m` buffer in row-major order (element
/// [row][col] at `row_major[row * m + col]`). A single H2D memcpy transfers
/// the whole trace, then an on-device transpose scatters each element into the
/// column-major LDE buffer (col c, element r → buf[c * lde_size + r]).  The
/// rest of the pipeline (NTT, Keccak, Merkle) is identical to the column-major
/// path.  The LDE output is also transposed back to row-major before D2H, so
/// the caller receives a flat `lde_size * m` row-major buffer and never needs
/// to call `columns_to_row_major` on the host.
///
/// Returns (merkle_nodes, GpuLdeBase handle, row-major LDE buffer).
pub fn coset_lde_batch_base_row_major_with_merkle_tree_keep(
    row_major: &[u64],
    n: usize,
    m: usize,
    blowup_factor: usize,
    ...
    let lde_size = n * blowup_factor;
    assert_u32_domain(lde_size, "coset_lde_batch_base_row_major lde_size");
    ...
    // H2D: single contiguous copy of the row-major trace.
    let src_dev = stream.clone_htod(row_major)?;

    // Allocate the column-major LDE buffer (m cols × lde_size, zero-padded).
    let mut buf = stream.alloc_zeros::<u64>(m * lde_size)?;

    // Transpose: row-major (n×m) → column-major with col_stride = lde_size.
    // Element [r][c] in src → buf[c * lde_size + r].
    launch_matrix_transpose_strided(
        stream.as_ref(),
        be,
        &src_dev,
        &mut buf,
    ...
    // iNTT → coset weights → forward NTT (identical to column-major path).
    launch_bit_reverse_batched(stream.as_ref(), be, &mut buf, n_u64, log_n, col_stride_u64, m_u32)?;
    run_batched_ntt_body(stream.as_ref(), &mut buf, inv_tw.as_ref(), n_u64, log_n, col_stride_u64, m_u32)?;
    ...
    run_batched_ntt_body(stream.as_ref(), &mut buf, fwd_tw.as_ref(), lde_u64, log_lde, col_stride_u64, m_u32)?;

    // Keccak + Merkle (identical to column-major path).
    ...
    let leaves_offset = KeccakCommit::FullTree.leaves_offset_bytes(lde_size);
    {
        ...
        launch_keccak_base(stream.as_ref(), &buf, col_stride_u64, m as u64, lde_u64, &mut leaves_view)?;
    }
    crate::merkle::build_inner_tree_levels(stream.as_ref(), be, &mut nodes_dev, lde_size)?;

    // Transpose LDE output back to row-major: buf (m cols × lde_size) → row-major (lde_size × m).
    // Element buf[c * lde_size + r] → row_major_out[r * m + c].
    // This is a (lde_size × m) matrix stored column-major → transpose to row-major.
    // Equivalently: matrix_transpose_strided(src=buf viewed as m×lde_size col-major,
    // dst=row_major_out, rows=lde_size, cols=m, out_stride=m).
    let mut row_major_dev = stream.alloc_zeros::<u64>(m * lde_size)?;
    launch_matrix_transpose_strided(
        stream.as_ref(),
        be,
        &buf,
    ...
    // D2H: merkle nodes + row-major LDE as single transfers.
    ...
    staging.ensure_capacity(m * lde_size, &be.ctx)?;
    let pinned = unsafe { staging.as_mut_slice(m * lde_size) };
    stream.memcpy_dtoh(&row_major_dev, pinned)?;
    stream.synchronize()?;
    let lde_row_major = pinned[..m * lde_size].to_vec();
    drop(staging);
    ...
    Ok((nodes_out, handle, lde_row_major))
}
