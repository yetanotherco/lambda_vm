...
pub fn batch_inverse_ext3(a: &[u64]) -> Result<Vec<u64>> {
    ...
    let cfg_combine = LaunchConfig {
        grid_dim: (((n as u32) + COMBINE_BLOCK - 1) / COMBINE_BLOCK, 1, 1),
...
fn invert_ext3_host(x: [u64; 3]) -> [u64; 3] {
    ...
    //   Let x = (a, b, c) representing a + b·w + c·w²
    //   Then x⁻¹ = (d, e, f) / N
    ...
    let bc = gl_mul(b, c);
    let d = gl_sub(gl_mul(a, a), gl_add(bc, bc)); // a² - 2bc
    let cc = gl_mul(c, c);
    let ab = gl_mul(a, b);
    let e = gl_sub(gl_add(cc, cc), ab); // 2c² - ab
    let bb = gl_mul(b, b);
    let ac = gl_mul(a, c);
    let f = gl_sub(bb, ac); // b² - ac
