//! GPU MEMW_A (aligned memory/register) main-column generation. Caller
//! pre-builds six parallel `Vec<u64>` arrays (length `num_rows`, padded);
//! `values`/`olds` are length `8 * num_rows` (row-major interleaving).
//!
...
pub static FAULT_MEMW_ALIGNED_TRACE_REMAINING_UNTIL_ERR: std::sync::atomic::AtomicI64 =
    std::sync::atomic::AtomicI64::new(-1);

#[cfg(feature = "test-faults")]
fn check_memw_aligned_trace_fault_injection() -> Result<()> {
    use std::sync::atomic::Ordering;
    let v = FAULT_MEMW_ALIGNED_TRACE_REMAINING_UNTIL_ERR.load(Ordering::Relaxed);
    if v < 0 {
        return Ok(());
    }
    let prev = FAULT_MEMW_ALIGNED_TRACE_REMAINING_UNTIL_ERR.fetch_sub(1, Ordering::Relaxed);
    if prev == 1 {
...
pub fn generate_memw_aligned_trace_dev(
    num_rows: usize,
    base_addresses: &[u64],
    timestamps: &[u64],
    old_timestamps: &[u64],
    ...
    #[cfg(feature = "test-faults")]
    check_memw_aligned_trace_fault_injection()?;
    ...
    assert!(num_cols >= 29, "memw_aligned table needs at least 29 columns");
    assert!(
    ...
    unsafe {
        stream
            .launch_builder(&be.generate_memw_aligned_trace_rows)
            .arg(&num_rows_u)
            .arg(&addr_dev)
            .arg(&ts_dev)
            .arg(&old_ts_dev)
