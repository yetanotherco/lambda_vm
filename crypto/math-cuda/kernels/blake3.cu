...
// Row-major ONE-ROW leaf hashing (S2, rows_per_leaf = 1): leaf `tid` hashes the
// single row `reverse_index(tid)`, columns `[col_start, col_end)` of the
// row-major buffer (`m` the full row stride). Byte stream = the CPU
// `commit_rows_bit_reversed_subset_with(.., 1)`. Twin of
// `keccak256_leaves_base_row_major_row_range`.
extern "C" __global__ void blake3_leaves_base_row_major_row_range(
    const uint64_t *data,
    uint64_t m,
    uint64_t col_start,
    uint64_t col_end,
    ...
    uint8_t *hashed_leaves_out)
{
    ...
    Blake3Chain h;
    h.init();
    for (uint64_t c = col_start; c < col_end; ++c) h.push_felt(row[c]);
    h.finalize(hashed_leaves_out + tid * 32);
}
