...
fn run_parity(
    ...
    let cpu_out = cpu_deep(
        &main_lde, &aux_lde, &h_lde, &h_ood, &trace_ood, &gammas_h, &gammas_tr, &inv_h, &inv_t,
        blowup_factor, domain_size,
    );
    ...
    // main_lde → col-major u64: m × lde_size
    ...
    // aux_lde → de-interleaved: (m*3) × lde_size
    ...
    // h_parts → de-interleaved: num_parts*3 × lde_size
    ...
    let gpu_raw = math_cuda::deep::deep_composition_ext3(
        &main_handle,
        aux_handle.as_ref(),
        &h_flat,
        &h_ood_flat,
