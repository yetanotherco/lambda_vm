...
//! the CPU commit are pinned by `merkle_root_parity` / `blake3_fused_parity`.
//!
...
fn hash_for(i: usize) -> DeviceHash {
...
/// `_ITERS` (5), `_HASH` (`keccak` | `blake3`), `_PREDEV` (unset: host
...
fn vram_arm() {
    ...
    let hash = match std::env::var("LAMBDA_VM_VRAM_ARM_HASH").as_deref() {
        Ok("blake3") => DeviceHash::Blake3,
        _ => DeviceHash::Keccak256,
    };
    let n = 1usize << log_n;
    let rows = n * blowup;
    ...
    println!(
        "vram_arm: 2^{log_n} rows x {cols} cols @ blowup {blowup}, {hash:?}, input {}",
