...
fn run_parity(log_n: u32, blowup: usize, num_parts: usize, seed: u64) {
    ...
    let mut outputs_slices: Vec<&mut [u64]> = outputs_raw
        .iter_mut()
    ...
    for log_n in 2u32..=5 {
        for &blowup in &[2usize, 4, 8] {
            for &parts in &[1usize, 2, 4] {
                run_parity(log_n, blowup, parts, 1000 + log_n as u64 * 31 + parts as u64);
            }
        }
    }
}
...
fn comp_poly_tree_medium() {
    for &(log_n, blowup, parts) in &[(10u32, 4usize, 4usize), (12, 2, 3)] {
        run_parity(log_n, blowup, parts, 2000 + log_n as u64 * 11 + parts as u64);
    }
}
