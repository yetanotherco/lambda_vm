...
mod tests {
    ...
    /// The claimed values are absorbed **before** the weights that combine
    /// them are drawn, which is the only thing stopping a prover from picking a
    /// value after seeing the weight it will be multiplied by.
    ///
    /// A proof cannot show this: take the absorption out of both sides and
    /// every test here still passes, because the two sides still agree on the
    /// weights they are wrong about together. The transcript can — two
    /// different claims have to give different weights, so the proof they
    /// produce over the same columns has to come out different.
    #[test]
    fn the_claimed_values_are_bound_to_the_weights() {
        let num_vars = 3;
        let (layout, columns) = stack_of(3, num_vars, 6);
        ...
        let honest = values(&columns, &at);
        let mut moved = honest.clone();
        moved[1] += FE::one();

        let rounds = |p: &StackedProof<F, F>| {
            p.polys
                .iter()
                .map(|e| {
                    e.rounds
                        .iter()
                        .flat_map(|r| r.sumcheck.iter().map(|s| s.evaluations.clone()))
                        .collect::<Vec<_>>()
                })
                .collect::<Vec<_>>()
        };
        ...
        assert_ne!(
            rounds(&prove_with(&honest)),
            rounds(&prove_with(&moved)),
            "a claimed value does not reach the weights"
        );
    }
