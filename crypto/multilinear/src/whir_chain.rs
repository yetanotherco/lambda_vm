...
mod tests {
    ...
    /// A value a round sends has to be in the transcript before the challenge
    /// that depends on it is drawn: the out-of-domain value before `gamma`, the
    /// final value before the queries.
    ///
    /// A proof cannot show this. Take any of those absorptions out of both
    /// sides and the two sides still agree, every proof still verifies, and
    /// every other test here still passes; what is gone is the prover's
    /// inability to pick the value after seeing the challenge. So the test is
    /// on the transcript: move the value, and the verifier has to end up
    /// somewhere else.
    #[test]
    fn what_a_round_sends_is_bound_to_the_transcript() {
        let num_vars = 6;
        let cfg = config(2);
        ...
        assert!(
            proof.rounds.len() > 1,
            "a single round has no successor to bind"
        );

        // Where the verifier ends up, whether or not it got there happily: a
        // rejection still says what went into the transcript on the way.
        let reached = |p: &ChainProof<F, F>| {
            let mut t = transcript();
            let _ = verify::<F, F, _>(p, &commitment.root(), &z, y, &domain, &cfg, &mut t);
            t.state()
        };
        let honest = reached(&proof);

        // Not the successor's root, and the reason is worth knowing: moving it
        // lands the verifier somewhere else whether or not it was absorbed,
        // because the openings are checked against it and a run that stops
        // early has absorbed less than one that does not. So a test shaped like
        // this one cannot tell the absorption apart from the Merkle check, and
        // the root's binding — that the out-of-domain point is drawn after the
        // successor is fixed — has no test here.

        let mut moved_ood = proof.clone();
        let ood = moved_ood.rounds[0].ood_value.as_mut().expect("ood value");
        *ood += FE::one();
        assert_ne!(
            reached(&moved_ood),
            honest,
            "the out-of-domain value is not in the transcript"
        );

        let mut moved_final = proof.clone();
        moved_final.final_value += FE::one();
        assert_ne!(
            reached(&moved_final),
            honest,
            "the final value is not in the transcript"
        );
    }
