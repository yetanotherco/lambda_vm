...
///
/// The two arms read the SAME 32-byte inner hash in different byte orders —
/// keccak's kernel as four LITTLE-endian lanes, RPX's as four BIG-endian felts
/// — so this names the arm AND the encoding together. Crossing them compiles,
/// runs, and searches for a nonce under a message the host never hashes.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum DeviceGrindKey {
    /// `math_cuda::grinding::generate_nonce_gpu`, little-endian lanes.
    Keccak256,
...
///
/// ⚠ **This replaced a central list, and the reason is the merge that created
/// this file.** The dispatch used to be `has_device_kernel::<D>()`, a `TypeId`
/// comparison against the two digests `crypto` happens to define. That is
/// correct exactly while every grinding hash lives here. It stopped being true
/// the moment the per-table branch merged in: its block path grinds with
/// `prover::lfm::algebraic_commit::AlgebraicDigest<RpxCommit>`, which `crypto`
/// cannot name at all — `prover` depends on this crate, not the reverse — so
/// the list returned false, the RPX grind fell to the host rayon search, and
/// nothing failed. It compiles, the proofs stay valid, and the only signal is a
/// counter that does not increment.
///
/// As an associated constant the arm travels with the configuration instead:
/// there is no list to be missing from, and
/// [`TranscriptHash::Digest`](crate::fiat_shamir::transcript_hash::TranscriptHash::Digest)
/// requires this bound, so a configuration that does not declare an arm does
/// not compile rather than silently taking the slow one.
///
/// `None` is a statement too — BLAKE3, RPO and Poseidon have no grind kernel,
/// and the CPU search is where they belong.
pub trait GrindDigest: Digest + OutputSizeUser<OutputSize = U32> + 'static {
    /// The device arm, or `None` for "the host search is the only one".
    const DEVICE_GRIND: Option<DeviceGrindKey>;
}

impl GrindDigest for crate::hash::platform_keccak::PlatformKeccak256 {
    const DEVICE_GRIND: Option<DeviceGrindKey> = Some(DeviceGrindKey::Keccak256);
}

impl GrindDigest for crate::hash::rpx::Rpx256Digest {
    const DEVICE_GRIND: Option<DeviceGrindKey> = Some(DeviceGrindKey::Rpx256);
}

impl GrindDigest for crate::hash::blake3::chain::Blake3Chain {
    /// No BLAKE3 grind kernel exists. `math-cuda` ships BLAKE3 leaf and parent
    /// kernels, but not a nonce search, so this is `None` rather than an
    /// omission.
    const DEVICE_GRIND: Option<DeviceGrindKey> = None;
}
...
///
/// ★ The dispatch is keyed on [`GrindDigest::DEVICE_GRIND`] — the arm the
/// digest's own configuration names, the host twin of the `DeviceHash` key the
/// commit path carries.
///
    ...
    D: GrindDigest,
{
    ...
    let Some(key) = D::DEVICE_GRIND else {
        return generate_nonce::<D>(seed, grinding_factor);
    };
    ...
    let found = match key {
        DeviceGrindKey::Keccak256 => math_cuda::grinding::generate_nonce_gpu(
            &inner_hash_lanes::<D>(seed, grinding_factor),
            grinding_factor,
        ),
        DeviceGrindKey::Rpx256 => math_cuda::grinding::generate_nonce_rpx_gpu(
