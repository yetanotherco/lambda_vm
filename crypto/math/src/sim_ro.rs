...
/// ROUND-2 increment C — `REGISTER_RO_LAYOUT` input (in-place reduced-opening
/// ABI). The PROOF-CONSTANT half of [`ReducedOpeningRowInput`]: the coeff-grid
/// column-pointer table, the transition window, the OOD dims, and the (constant)
/// per-slice column counts. Registered ONCE per proof; the executor caches it in
/// a thread-local. Thereafter each `REDUCED_OPENING_ROW_INPLACE` ecall passes
/// only `{row_idx, evals_ptr, out_ptr}`, where `evals_ptr` points at the six
/// per-query eval-slice base pointers (see [`REDUCED_OPENING_INPLACE_EVALS`]) —
/// killing the per-query struct fill + col-ptr gather the Level A ABI repeats for
/// every query. MEASUREMENT-ONLY, never proven.
#[repr(C)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ReducedOpeningLayout {
    /// Array of `ood_width` column-data pointers (`coeff[col][row]` at
    /// `col_ptrs[col] + row*24`); built ONCE per proof from `trace_term_coeffs`.
    pub coeff_col_ptrs_ptr: u64,
    /// `next_row_cols` transition window (g·z pruning), `usize` each.
    pub next_row_cols_ptr: u64,
    pub next_row_cols_len: u64,
    /// Total OOD table width = coeff grid column count.
    pub ood_width: u64,
    /// g·z pruning threshold.
    pub step_size: u64,
    /// Constant per-slice column counts (same for every query — the trace
    /// widths). Regular point, then symmetric point.
    pub precomputed_len: u64,
    pub main_len: u64,
    pub aux_len: u64,
    pub precomputed_sym_len: u64,
...
/// Number of `u64` fields in [`ReducedOpeningLayout`].
pub const REDUCED_OPENING_LAYOUT_FIELDS: usize = 11;

/// Number of per-query eval-slice base pointers a `REDUCED_OPENING_ROW_INPLACE`
/// ecall reads from `evals_ptr` (6 `u64`, in order): `precomputed_ptr`,
/// `main_ptr`, `aux_ptr`, `precomputed_sym_ptr`, `main_sym_ptr`, `aux_sym_ptr`.
/// The slices live where the verifier already holds them (the opening view), so
/// the guest passes their existing addresses — no per-row data copy.
pub const REDUCED_OPENING_INPLACE_EVALS: usize = 6;
