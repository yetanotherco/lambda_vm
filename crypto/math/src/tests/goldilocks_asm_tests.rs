...
mod extension_asm_tests {
    ...
    // Verify all alternatives produce the same results as the current implementation

    use crate::field::fields::fft_friendly::goldilocks_extensions_asm::{
        mul_by_7_u128, fp2_mul_direct, fp2_mul_fused,
        fp2_mul_karatsuba_delayed, fp2_mul_karatsuba_u128_mul7,
        fp3_mul_delayed,
    };

    #[test]
    fn test_mul_by_7_alternatives() {
        let test_values = [5u64, 100, GOLDILOCKS_PRIME - 1, 1, 0, 123456789,
                          0xFFFFFFFF, 0x123456789ABCDEF];
        for a in test_values {
            let current = mul_by_7(a);
            let u128_ver = mul_by_7_u128(a);
            assert_eq!(
                canonicalize(current), canonicalize(u128_ver),
                "mul_by_7 mismatch for a={}", a
            );
        }
    }
    ...
    #[test]
    fn test_fp2_mul_alternatives_match() {
        let test_pairs = [
            ([3u64, 4u64], [1u64, 2u64]),
            ([5u64, 7u64], [13u64, 17u64]),
            ([GOLDILOCKS_PRIME - 1, 1], [2, GOLDILOCKS_PRIME - 2]),
            ([0, 1], [1, 0]),
            ([123456789, 987654321], [111111111, 222222222]),
        ];

        for (a, b) in test_pairs {
            let current = fp2_mul(a, b);
            let direct = fp2_mul_direct(a, b);
            let fused = fp2_mul_fused(a, b);
            let karatsuba_delayed = fp2_mul_karatsuba_delayed(a, b);
            let karatsuba_u128 = fp2_mul_karatsuba_u128_mul7(a, b);

            // All should match the current implementation
            assert_eq!(
                canonicalize(current[0]), canonicalize(direct[0]),
                "fp2_mul_direct c0 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[1]), canonicalize(direct[1]),
                "fp2_mul_direct c1 mismatch for a={:?}, b={:?}", a, b
            );

            assert_eq!(
                canonicalize(current[0]), canonicalize(fused[0]),
                "fp2_mul_fused c0 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[1]), canonicalize(fused[1]),
                "fp2_mul_fused c1 mismatch for a={:?}, b={:?}", a, b
            );

            assert_eq!(
                canonicalize(current[0]), canonicalize(karatsuba_delayed[0]),
                "fp2_mul_karatsuba_delayed c0 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[1]), canonicalize(karatsuba_delayed[1]),
                "fp2_mul_karatsuba_delayed c1 mismatch for a={:?}, b={:?}", a, b
            );

            assert_eq!(
                canonicalize(current[0]), canonicalize(karatsuba_u128[0]),
                "fp2_mul_karatsuba_u128 c0 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[1]), canonicalize(karatsuba_u128[1]),
                "fp2_mul_karatsuba_u128 c1 mismatch for a={:?}, b={:?}", a, b
            );
        }
    }

    #[test]
    fn test_fp3_mul_delayed_matches() {
        let test_pairs = [
            ([1u64, 2u64, 3u64], [4u64, 5u64, 6u64]),
            ([5u64, 7u64, 11u64], [13u64, 17u64, 19u64]),
            ([GOLDILOCKS_PRIME - 1, 1, 2], [3, GOLDILOCKS_PRIME - 2, 1]),
            ([0, 1, 0], [1, 0, 1]),
        ];

        for (a, b) in test_pairs {
            let current = fp3_mul(a, b);
            let delayed = fp3_mul_delayed(a, b);

            assert_eq!(
                canonicalize(current[0]), canonicalize(delayed[0]),
                "fp3_mul_delayed c0 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[1]), canonicalize(delayed[1]),
                "fp3_mul_delayed c1 mismatch for a={:?}, b={:?}", a, b
            );
            assert_eq!(
                canonicalize(current[2]), canonicalize(delayed[2]),
                "fp3_mul_delayed c2 mismatch for a={:?}, b={:?}", a, b
            );
        }
    }
}
