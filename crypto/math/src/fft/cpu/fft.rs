...
mod tests {
    ...
    proptest! {
        ...
        fn test_nr_2radix_fft_matches_naive_eval(coeffs in field_vec(8)) {
            ...
            let twiddles = get_powers_of_primitive_root::<F>(order.into(), (1 << order) / 2, RootsConfig::BitReverse).unwrap();
        ...
        fn test_rn_2radix_fft_matches_naive_eval(coeffs in field_vec(8)) {
            ...
            let order = coeffs.len().trailing_zeros();
            let twiddles = get_powers_of_primitive_root::<F>(order.into(), (1 << order) / 2, RootsConfig::Natural).unwrap();
