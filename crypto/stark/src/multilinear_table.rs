    ...
    T: crypto::fiat_shamir::is_transcript::IsTranscript<E>,
{
    ...
    // Before the challenges, so the tables cannot be chosen after seeing them.
    //
    // Untested, and not for want of trying: a root is also what the openings
    // are checked against, so moving one lands the verifier elsewhere whether
    // or not it was absorbed, and a test that compares where the verifier ends
    // up cannot tell the two apart. The same is true of `bus_output` below,
    // which the GKR check consumes. Separating them needs a seam that does not
    // exist yet.
