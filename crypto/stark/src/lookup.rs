    ...
    E: IsField + Send + Sync,
{
    ...
    // Compute L = sum of all row sums (table contribution)
    let table_contribution: FieldElement<E> =
        row_sums.iter().fold(FieldElement::zero(), |acc, x| acc + x);

    // offset_per_row = L / N (for circular constraint)
    let n = FieldElement::<E>::from(trace_len as u64);
    let offset_per_row = &table_contribution * n.inv().unwrap();

    // Adjust row sums: each becomes (row_sum - L/N)
    // Prefix sum gives: acc[i] = Σ_{j=0..=i} (row_sums[j] - L/N)
    // Result: acc[N-1] = L - N*(L/N) = 0
    let adjusted: Vec<FieldElement<E>> =
        row_sums.into_iter().map(|s| s - &offset_per_row).collect();

    // Prefix sum
    #[cfg(feature = "parallel")]
    let acc = parallel_prefix_sum(&adjusted);
