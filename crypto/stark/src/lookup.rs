...
> AirWithBuses<F, E, B, PI>
{
    ...
    /// - Columns 0..num_committed_groups-1: Committed term columns (batched triples)
    /// - Last column: Accumulated column (running sum + 1-3 absorbed interactions)
    ///
    /// The last 1-3 interactions are "absorbed" into the accumulated constraint
    /// by clearing denominators, eliminating one committed term column per table.
    ///
    /// Total aux columns = ⌈N/3⌉ where N is the number of interactions.
    pub fn new(
        ...
        // Split interactions: committed groups of 3 get term columns, remainder absorbed
        let (num_committed_groups, absorbed) = if num_interactions == 0 {
            (0, vec![])
        } else if num_interactions <= 3 {
            ...
            let remainder = num_interactions % 3;
            let absorbed_count = if remainder == 0 { 3 } else { remainder };
            (
                (num_interactions - absorbed_count) / 3,
                auxiliary_trace_build_data.interactions[num_interactions - absorbed_count..]
                    .to_vec(),
            )
        };

        // Create batched term constraints for committed groups of 3 only
        for group_idx in 0..num_committed_groups {
            let base = group_idx * 3;
            let constraint = LookupBatchedTermConstraint3::new(
        ...
        // Layout: num_committed_groups term columns + 1 accumulated = ⌈N/3⌉
    ...
    PI: Send + Sync,
{
    ...
    fn build_auxiliary_trace(
        ...
        // Split interactions: committed groups of 3 get term columns, remainder absorbed (virtual)
        let (num_committed_groups, absorbed_count) = if num_interactions <= 3 {
            (0, num_interactions)
        } else {
            let remainder = num_interactions % 3;
            let absorbed_count = if remainder == 0 { 3 } else { remainder };
            ((num_interactions - absorbed_count) / 3, absorbed_count)
        };

        // Compute committed term columns in parallel (batched groups of 3 only)
        #[cfg(feature = "parallel")]
        let committed_columns: Vec<Vec<FieldElement<E>>> = (0..num_committed_groups)
            .into_par_iter()
            .map(|i| {
                let base = i * 3;
                compute_logup_batched_term_column_3(
...
/// Computes a batched term column for three interactions sharing one aux column.
///
/// Each row contains: `term[i] = Σ_{k=0..2} sign_k * m_k[i] / fp_k[i]`
///
/// Uses a single batch inversion for all fingerprint vectors (3*N elements).
#[allow(clippy::needless_range_loop)]
#[allow(clippy::too_many_arguments)]
fn compute_logup_batched_term_column_3<F, E>(
    ...
    E: IsField + Send + Sync,
{
    ...
    let interactions = [interaction_a, interaction_b, interaction_c];
    ...
    // Concatenate all 3 fingerprint vectors for a single batch inversion
    let mut all_fingerprints: Vec<FieldElement<E>> = Vec::with_capacity(3 * trace_len);
    ...
    // Single batch inversion for all 3*N fingerprints
...
/// Computes a virtual term column for 1-3 absorbed interactions.
///
    ...
    E: IsField + Send + Sync,
{
    ...
    assert!((1..=3).contains(&n), "absorbed count must be 1-3");
...
/// Constraint for a batched triple of interactions sharing one aux column.
///
/// Verifies: `c = Σ_{k=0..2} sign_k * m_k / fp_k`
///
/// Clearing denominators:
/// `c * fp_a * fp_b * fp_c - Σ(sign_k * m_k * ∏_{j≠k} fp_j) = 0`
///
/// Degree 4: c (aux) × fp_a × fp_b × fp_c (each linear in main).
struct LookupBatchedTermConstraint3 {
    interaction_a: BusInteraction,
    interaction_b: BusInteraction,
    interaction_c: BusInteraction,
...
impl LookupBatchedTermConstraint3 {
    pub fn new(
        interaction_a: BusInteraction,
        interaction_b: BusInteraction,
        interaction_c: BusInteraction,
...
impl<F, E> TransitionConstraint<F, E> for LookupBatchedTermConstraint3
where
    F: IsFFTField + IsSubFieldOf<E> + Send + Sync,
    E: IsField + Send + Sync,
{
    ...
    fn evaluate(
        ...
        #[allow(clippy::too_many_arguments)]
        fn evaluate_batched_term_constraint_3<A: IsSubFieldOf<B>, B: IsField>(
            step: &TableView<A, B>,
            term_column_idx: usize,
            interaction_a: &BusInteraction,
            interaction_b: &BusInteraction,
            ...
            // ∏_{j≠k} fp_j:
            let no_a = &fp_b * &fp_c; // fp_b * fp_c
            let no_b = &fp_a * &fp_c; // fp_a * fp_c
            let no_c = fab; // fp_a * fp_b (already computed)

            c * fabc - m_a * sign_a * no_a - m_b * sign_b * no_b - m_c * sign_c * no_c
        }
        ...
        let res = match evaluation_context {
            TransitionEvaluationContext::Prover {
                ...
                logup_alpha_powers,
                ..
            } => evaluate_batched_term_constraint_3(
...
/// plus 1-3 "absorbed" interactions whose terms are verified inline (not committed).
///
/// Degree = 1 + absorbed.len():
///   1 absorbed → degree 2, 2 → degree 3, 3 → degree 4
struct LookupAccumulatedConstraint {
    ...
    acc_column_idx: usize,
    /// 1-3 interactions absorbed into this constraint (not committed as columns)
...
{
    fn degree(&self) -> usize {
        1 + self.absorbed.len() // 2 for 1, 3 for 2, 4 for 3 absorbed
    }
    ...
    fn evaluate(
        ...
        fn evaluate_accumulated_constraint<A: IsSubFieldOf<B>, B: IsField>(
            ...
            match absorbed.len() {
                ...
                _ => unreachable!("absorbed must contain 1-3 interactions"),
            }
        }
