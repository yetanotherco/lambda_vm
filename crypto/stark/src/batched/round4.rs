...
use crate::config::{BatchedMerkleTreeBackend, Commitment, FriLayerMerkleTreeBackend};
...
pub struct BatchedFriCommit<E: IsField + 'static>
where
    FieldElement<E>: AsBytes + Sync + Send,
{
    pub layers: Vec<FriLayer<E, FriLayerMerkleTreeBackend<E>>>,
...
/// Returns `false` on every malformed input; it never panics.
#[allow(clippy::too_many_arguments)]
pub fn verify_batched_fri_query<F, E>(
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    for i in 0..layout.num_committed {
        ...
        openings_ok &= verify_layer_opening::<E>(
            &layer_roots[i],
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    verify_merkle_path::<BatchedMerkleTreeBackend<E>>(auth_path, root, index >> 1, &leaf)
}
...
pub(crate) mod tests {
    ...
    pub(crate) fn standalone_refs(commit: &BatchedFriCommit<F>) -> Vec<Option<&[FE]>> {
        commit
            .standalone_coeffs
            .iter()
            .map(|c| c.as_deref())
    ...
    pub(crate) fn commit_fixture(
        ...
        commit_batched_fri::<F, F, Transcript, _>(
            transcript,
            &heights,
            &widths,
            |alpha, plan| {
    ...
    fn honest_batched_queries_verify() {
        ...
        let decommitments = crate::fri::query_phase::<F>(&commit.layers, &commit.iotas);
