...
/// commitment half.
pub fn verify_epoch_commitments<Field, FieldExtension, PI>(
    airs: &[&dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>],
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    shape: &EpochShape,
    ...
    FieldElement<FieldExtension>: AsBytes + Sync + Send,
{
    ...
    if params.grinding_factor > 0 {
        ...
        if !crate::grinding::is_valid_nonce(
    ...
    for (query, iota) in challenges.fri.iotas.iter().copied().enumerate() {
        ...
        if !round_authenticates::<FieldExtension>(
            &proof.parts_root,
            &opening.parts,
            &shape.parts,
            iota,
        ...
        for (k, &t) in shape.prep.tables.iter().enumerate() {
            ...
            let leaf_hash = <BatchedMerkleTreeBackend<Field> as crypto::merkle_tree::traits::IsStreamingLeafBackend<Field>>::hash_data_from_slices(
                &o.evaluations,
                &o.evaluations_sym,
            );
            if !crypto::merkle_tree::proof::verify_merkle_path_from_leaf_hash::<BatchedMerkleTreeBackend<Field>>(
                &o.proof.merkle_path,
        ...
        match (
            ...
            (Some(c), Some(root), Some(o)) => {
                ...
                let leaf_hash =
                    <BatchedMerkleTreeBackend<Field> as crypto::merkle_tree::traits::IsStreamingLeafBackend<
                        Field,
        ...
        match (proof.aux_root.as_ref(), opening.aux.as_ref()) {
            (Some(root), Some(o)) => {
                if !round_authenticates::<FieldExtension>(root, o, &shape.aux, iota, h_max) {
                    return false;
                }
            }
            (None, None) => {}
    ...
    FieldElement<C>: AsBytes + Sync + Send,
{
    ...
    MixedMmcs::<C>::verify_batch(root, iota, opening, &round.heights(), &round.widths())
}
...
pub fn verify_epoch_fri<Field, FieldExtension, PI, V>(
    airs: &[&dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>],
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    shape: &EpochShape,
    params: &EpochFriParams,
...
where
    ...
    V: crate::verifier::IsStarkVerifier<Field, FieldExtension, PI> + ?Sized,
{
    let h_max = shape.h_max();
    ...
    for (table, air) in airs.iter().enumerate() {
        match table_deep_pairs::<Field, FieldExtension, PI, V>(
    ...
    for (query, iota) in challenges.fri.iotas.iter().copied().enumerate() {
        ...
        if !crate::batched::round4::verify_batched_fri_query::<Field, FieldExtension>(
            &proof.fri_layer_roots,
...
fn table_deep_pairs<Field, FieldExtension, PI, V>(
    table: usize,
    air: &dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>,
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    shape: &EpochShape,
...
pub fn verify_epoch_constraints<Field, FieldExtension, PI, V>(
    airs: &[&dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>],
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    challenges: &EpochChallenges<FieldExtension>,
...
pub fn multi_verify_batched<Field, FieldExtension, PI, V, T>(
    airs: &[&dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>],
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    transcript: &mut T,
    expected_bus_balance: &FieldElement<FieldExtension>,
...
where
    ...
    T: IsStarkTranscript<FieldExtension, Field>,
{
    multi_verify_batched_carved::<Field, FieldExtension, PI, V, T>(
        airs,
        proof,
        transcript,
...
pub fn multi_verify_batched_carved<Field, FieldExtension, PI, V, T>(
    airs: &[&dyn AIR<Field = Field, FieldExtension = FieldExtension, PublicInputs = PI>],
    proof: &BatchedMultiProof<Field, FieldExtension, PI>,
    transcript: &mut T,
    expected_bus_balance: &FieldElement<FieldExtension>,
    ...
    T: IsStarkTranscript<FieldExtension, Field>,
{
    ...
    verify_epoch_commitments::<Field, FieldExtension, PI>(airs, proof, &shape, &params, &challenges)
        && verify_epoch_constraints::<Field, FieldExtension, PI, V>(
            airs,
            proof,
            &challenges,
            expected_bus_balance,
        )
        && verify_epoch_fri::<Field, FieldExtension, PI, V>(
        airs,
        proof,
        &shape,
        &params,
