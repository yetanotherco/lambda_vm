...
use crate::config::{
    field_element_to_fps, FriLayerMerkleTreeBackendInner, FriLayerMerkleTreeInner,
};
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    let mut fri_layer_list = Vec::with_capacity(number_layers);
    let mut current_layer: FriLayer<E, FriLayerMerkleTreeBackendInner>;
    let mut current_poly = p_0;
    ...
    for _ in 1..number_layers {
        ...
        fold_polynomial_doubled_inplace(&mut current_poly, &zeta);
        current_layer = new_fri_layer(&current_poly, &coset_offset, domain_size);
        // Copy the root (Fp field element for Poseidon2)
        let new_data = current_layer.merkle_tree.root;
        fri_layer_list.push(current_layer);

        // >>>> Send commitment: [pₖ]
        // Convert Fp (Commitment) to bytes for transcript
        let bytes = crate::config::commitment_to_bytes(&new_data);
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    // For Poseidon2, encode each field element into a vector of Fp limbs.
    // Each leaf is the concatenation of two evaluations' limbs.
    let leaves: Vec<Vec<crate::config::PoseidonFp>> = evaluation
        .chunks_exact(2)
        .map(|chunk| {
    ...
    let merkle_tree = FriLayerMerkleTreeInner::build(&leaves).unwrap();
