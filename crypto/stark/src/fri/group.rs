...
/// `leaf = p >> d_j` by `fri_checks[j]` (path `paths(j)`; the exact depth and
/// any cap are the check's, built once per tree from the verifier's
/// constants), the slot check `group[p & (2^{d_j} − 1)] == v` holds, and `v`
/// becomes the group fold with layer `j`'s challenge; finally
/// `terminal[p] == v`.
///
...
{
    if fri_checks.len() != layout.num_committed
    ...
    for (j, &d) in layout.schedule.iter().enumerate() {
        ...
        if !fri_checks[j].verify::<B>(query, paths(j), leaf, leaf_hash) && !mutated(2) {
            ok = false;
        }
