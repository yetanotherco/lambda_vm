...
use crate::proof::view::{MixedOpeningView, PolynomialOpeningsView};
...
/// group of openings (in the given order) into one digest. Reads the row pairs
/// through borrowed [`PolynomialOpeningsView`]s so the batched verifier hashes
/// straight out of the (owned or archived) proof — only the contiguous hash
/// buffer is materialized, never a copy of the proof's opening vectors.
fn hash_group_openings<E>(group: &[PolynomialOpeningsView<E>]) -> Commitment
where
    E: IsField,
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    where
        E::BaseType: math::field::element::NativeArchived,
    {
        Self::verify_batch_view(
            root,
            iota,
            MixedOpeningView::Owned(opening),
            heights,
            widths,
        )
    }

    /// Zero-copy counterpart of [`Self::verify_batch`]: authenticates a batched
    /// opening read through a borrowed [`MixedOpeningView`] (owned or
    /// archived-in-place), so the batched verifier never deserializes the
    /// per-query opening. Byte-identical to the owned path — the owned wrapper
    /// above just forwards `MixedOpeningView::Owned`.
    pub fn verify_batch_view(
        root: &Commitment,
        iota: usize,
        opening: MixedOpeningView<'_, E>,
        heights: &[usize],
        widths: &[usize],
    ) -> bool
    where
        E::BaseType: math::field::element::NativeArchived,
    {
        let num_matrices = opening.per_matrix_len();
        if num_matrices != heights.len() || heights.len() != widths.len() || heights.is_empty() {
            return false;
        }
        // Per-matrix views (borrowed; no evaluation copy). Collected once and
        // reused by the base + inject groups below.
        let per_matrix: Vec<PolynomialOpeningsView<E>> =
            (0..num_matrices).map(|m| opening.per_matrix(m)).collect();
        ...
        for (o, w) in per_matrix.iter().zip(widths.iter()) {
        ...
        for (level, sibling) in merkle_path.iter().enumerate() {
            ...
            let inject_group: Vec<PolynomialOpeningsView<E>> = per_matrix
                .iter()
                .zip(heights.iter())
