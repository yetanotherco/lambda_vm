...
//! Plonky3). Mirrors Plonky3's `MerkleTreeMmcs`, adapted to the concrete keccak
//! commitment backends and to the row-pair `(x, -x)` leaf layout (#735).
//!
...
//! count `2^(h-1)`).
//!
//! Hashing (`H = <BatchedMerkleTreeBackend<E>>::hash_data` over a `Vec` of field
//! elements; `C = <BatchedMerkleTreeBackend<E>>::hash_new_parent`, the 2-input
//! compression — the same two functions, on the same backend, that the existing
//! per-table tree uses):
//!
...
//!
//! Because the leaf and parent hashes come from `BatchedMerkleTreeBackend<E>` —
//! the backend the per-table row-pair tree already commits with — a single-matrix `MixedMmcs` is
...
//!
//! A `LeafSource` serving rows from disk, device memory or recomputation is the
//! escape for that base-group residency: it lets a caller stream a matrix in,
//! hash it, and drop it without holding the whole group in RAM at once.
//!
//! (A streaming, incremental-leaf-hasher builder that keeps one hasher per leaf
//! and absorbs matrices as they arrive is planned but not part of this phase.)
...
/// A committed mixed-height, row-pair MMCS under the concrete keccak commitment
/// backends. Stores ONLY the digest layers (to serve the shared authentication path)
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    pub fn commit<S: LeafSource<E> + Sync>(source: &S) -> Self {
        ...
        for h in (1..=h_max).rev() {
            ...
            group_digests[h] = Some(crate::par::par_map_collect(0..1usize << (h - 1), |k| {
                hash_group_leaf::<E, S>(source, &group, k)
            }));
        }
    ...
    /// every group leaf in one pass; a future streaming builder would reach it
    /// having hashed them incrementally, matrix by matrix. That the two produce
    /// the same tree is therefore a property of calling one function, not a
    /// coincidence two code paths have to be shown to share.
    fn from_group_digests(
        ...
        while layers[i].len() > 1 {
            ...
            let next: Vec<Commitment> = crate::par::par_map_collect(0..next_len, |j| {
                let parent = compress::<E>(&cur[2 * j], &cur[2 * j + 1]);
                match &injected {
                    Some(digests) => compress::<E>(&parent, &digests[j]),
                    None => parent,
                }
            });
            layers.push(next);
    ...
    // Its consumer is #951's device-commit canary test, which this branch does
    // not bring over. Kept so the port stays a copy of the original.
    #[cfg(test)]
    #[allow(dead_code)]
    ...
    /// readable at once — the same `O(N)` residency a streaming commit path
    ...
    pub fn verify_batch(
        ...
        let base_group: Vec<&PolynomialOpenings<E>> = opening
            ...
            .map(|(o, _)| o)
            .collect();
        let mut acc = hash_group_openings::<E>(&base_group);
        ...
        for level in 0..(h_max - 1) {
            ...
            if !inject_group.is_empty() {
                let inj = hash_group_openings::<E>(&inject_group);
...
/// One leaf hasher of the batched (keccak) leaf backend.
type LeafHasherOf<E> = <BatchedMerkleTreeBackend<E> as IsStreamingLeafBackend<E>>::LeafHasher;
...
pub struct StreamingMmcsBuilder<E: IsField + 'static>
where
    FieldElement<E>: AsBytes + Sync + Send,
{
    dims: Vec<(usize, usize)>,
    ...
    pending: Vec<Option<Vec<LeafHasherOf<E>>>>,
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
        ...
        let pending = (0..=h_max)
            .map(|h| {
                (remaining[h] > 0).then(|| {
                    (0..1usize << (h - 1))
                        .map(|_| {
                            <BatchedMerkleTreeBackend<E> as IsStreamingLeafBackend<E>>::leaf_hasher(
                            )
                        })
                        .collect()
                })
    ...
    pub fn finish(self) -> MixedMmcs<E> {
        assert_eq!(
            self.next,
            self.dims.len(),
...
mod tests {
    ...
    /// ★ The backward-compatibility statement: a single-matrix MMCS IS the
    /// existing per-table row-pair tree. It holds by construction — both go
    /// through `BatchedMerkleTreeBackend<E>`'s `hash_data` / `hash_new_parent` —
    /// and this pins that no second leaf encoding crept in.
    ///
    /// Both sides use the SAME concrete keccak backend, which is what makes the
    /// comparison meaningful: `commit_bit_reversed` commits through
    /// `BatchedMerkleTreeBackend`, so a hash difference here would be a layout
    /// difference, not a hash-configuration mismatch.
    #[test]
    ...
    fn single_matrix_fp3_root_matches_existing_row_pair_tree() {
        ...
        let mmcs = MixedMmcs::<Fp3>::commit(&src);
        assert_eq!(
            mmcs.root(),
            existing_root,
        ...
        for iota in 0..(1usize << (log_height - 1)) {
            let opening = mmcs.open_batch(iota, &src);
            assert!(MixedMmcs::<Fp3>::verify_batch(
                &mmcs.root(),
                iota,
                &opening,
                &heights,
    ...
    impl<'a, E: IsField> Tracing<'a, E> {
        ...
        // Retained for a future streaming-commit phase's residency tests; the
        // one kept `commit` test drives residency via `materialize_all` only.
        #[allow(dead_code)]
    ...
    fn streaming_builder_commits_the_same_tree_as_commit() {
        ...
        let mut builder = StreamingMmcsBuilder::<GoldilocksField>::new(&dims);
        for m in 0..dims.len() {
    ...
    fn leaf_hasher_splits_anywhere_and_matches_hash_data() {
        ...
        where
            FieldElement<E>: AsBytes + Sync + Send,
        {
            let expected = <BatchedMerkleTreeBackend<E> as IsMerkleTreeBackend>::hash_data(&leaf);
    ...
    fn from_heap_nodes_rebuilds_the_same_tree() {
        ...
        assert_eq!(
            rebuilt.root(),
            m.root(),
            "root must survive the heap round-trip"
        );
