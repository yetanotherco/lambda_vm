...
mod tests {
    ...
    fn streaming_builder_commits_the_same_tree_as_commit() {
        ...
        let mut builder = StreamingMmcsBuilder::<GoldilocksField, KeccakStarkHash>::new(&dims);
        for m in 0..dims.len() {
    ...
    fn leaf_hasher_splits_anywhere_and_matches_hash_data() {
        ...
        {
            let expected =
                <<KeccakStarkHash as StarkHash>::Batched<E> as IsMerkleTreeBackend>::hash_data(
                    &leaf,
                );
            for split in 0..=leaf.len() {
                let mut hasher =
                    <<KeccakStarkHash as StarkHash>::Batched<E> as IsStreamingLeafBackend<E>>::leaf_hasher();
