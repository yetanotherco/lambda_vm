...
/// single scalar.
///
/// Layer trees are built with `FriLayerMerkleTreeBackend`, the same commitment
/// backend the unbatched [`crate::fri::commit_phase_from_evaluations`] uses — so a
/// batched prover and the verifier that authenticates its openings through
/// `BatchedMerkleTreeBackend` agree on the hash by naming one backend, not by two
/// call sites coinciding.
...
) -> (Vec<FieldElement<E>>, Vec<FriLayer<E, FriLayerMerkleTreeBackend<E>>>)
where
    ...
    FieldElement<E>: AsBytes + Sync + Send,
{
    ...
    for _ in 0..layout.num_committed {
        ...
        let leaves: Vec<[FieldElement<E>; 2]> = running
            ...
            .collect();
        let merkle_tree = MerkleTree::<FriLayerMerkleTreeBackend<E>>::build(&leaves)
...
mod tests {
    ...
    fn batched_commit_phase_first_layer_matches_manual_fold_and_inject() {
        ...
        let (_coeffs, layers) = batched_commit_phase::<_, _, _>(
            combined,
            &mut transcript,
            &coset_offset,
            blowup_log,
    ...
    fn single_bucket_terminal_matches_the_unbatched_commit_phase() {
        ...
        let (batched_coeffs, batched_layers) = batched_commit_phase::<_, _, _>(
            combined,
            &mut t_batched,
            &coset_offset,
            blowup_log,
    ...
    fn terminal_is_floored_at_the_shortest_codeword() {
        ...
        let (coeffs, layers) = batched_commit_phase::<_, _, _>(
            combined,
            &mut transcript,
            &coset_offset,
            blowup_log,
