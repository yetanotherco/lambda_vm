...
pub fn proof_vectors<H: StarkHash>(hash_name: &str) -> Vec<VectorFile> {
    ...
    for (fmt_name, format) in proof_formats() {
        out.extend(proof_files::<H>(hash_name, fmt_name, format, "d_proof"));
    }
    out
}
...
/// (e) The S2 proofs under hash `H`: the (d) shape proved with one-row
...
pub fn one_row_proof_vectors<H: StarkHash>(hash_name: &str) -> Vec<VectorFile> {
    ...
    for (fmt_name, format) in one_row_proof_formats() {
        out.extend(proof_files::<H>(hash_name, fmt_name, format, "e_proof"));
    }
    out
}
