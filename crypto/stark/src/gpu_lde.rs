...
/// Row-major variant of [`try_expand_leaf_and_tree_batched_keep`].
///
/// Takes the trace as a flat row-major slice (`n * m` elements) instead of
/// a `Vec<Vec<FieldElement>>`.  A single H2D memcpy replaces the per-column
/// copy, and an on-device transpose scatters elements into column-major layout
/// before the NTT.  The LDE output is transposed back to row-major on-device
/// before D2H, eliminating the `columns_to_row_major` host copy entirely.
///
/// Returns `(MerkleTree, GpuLdeBase handle, row-major LDE buffer)`.
    ...
    B: IsMerkleTreeBackend<Node = [u8; 32]>,
{
    ...
    // SAFETY: E == GoldilocksField checked above; FieldElement<E> is repr(transparent) over u64.
    ...
    let result = math_cuda::lde::coset_lde_batch_base_row_major_with_merkle_tree_keep(
        raw,
        n,
        m,
        blowup_factor,
    ...
    let (nodes_bytes, handle, lde_row_major_u64) = match result {
        Ok(t) => t,
        Err(_) => return None,
    };

    // Re-interpret the u64 LDE output as FieldElement<E>.
    // SAFETY: E == GoldilocksField, FieldElement<E> is repr(transparent) over u64.
    let lde_row_major: Vec<FieldElement<E>> = unsafe {
        let len = lde_row_major_u64.len();
        let ptr = lde_row_major_u64.as_ptr() as *const FieldElement<E>;
        let slice = from_raw_parts(ptr, len);
        // Prevent double-free: forget the original Vec after extracting the data.
        std::mem::forget(lde_row_major_u64);
        slice.to_vec()
    };
    ...
    let nodes: Vec<[u8; 32]> = nodes_bytes
        ...
        .collect();
    let tree = MerkleTree::<B>::from_precomputed_nodes(nodes)?;
    Some((tree, handle, lde_row_major))
}
