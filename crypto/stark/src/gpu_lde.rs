...
/// STORE-table GPU dispatch counter.
pub(crate) static GPU_STORE_TRACE_CALLS: AtomicU64 = AtomicU64::new(0);
pub fn gpu_store_trace_calls() -> u64 {
    GPU_STORE_TRACE_CALLS.load(Ordering::Relaxed)
}

/// Prover-crate wrapper for the GPU STORE-trace generator. `flags` is bit-
/// packed per row: bit0=WRITE2 bit1=WRITE4 bit2=WRITE8 bit3=MU.
/// `values` is one u64 per row — kernel does byte decomp.
pub fn try_generate_store_trace_gpu_raw(
    num_rows: usize,
    base_addresses: &[u64],
    timestamps: &[u64],
    values: &[u64],
    ...
    let raw = math_cuda::store_trace::generate_store_trace_dev(
        num_rows,
        base_addresses,
        timestamps,
        values,
...
/// SHIFT-table GPU dispatch counter.
pub(crate) static GPU_SHIFT_TRACE_CALLS: AtomicU64 = AtomicU64::new(0);
pub fn gpu_shift_trace_calls() -> u64 {
    GPU_SHIFT_TRACE_CALLS.load(Ordering::Relaxed)
}

/// MEMW_A-table GPU dispatch counter.
...
/// CPU32-table GPU dispatch counter.
...
/// MEMW-table GPU dispatch counter.
pub(crate) static GPU_MEMW_TRACE_CALLS: AtomicU64 = AtomicU64::new(0);
pub fn gpu_memw_trace_calls() -> u64 {
    GPU_MEMW_TRACE_CALLS.load(Ordering::Relaxed)
}

/// KECCAK-table GPU dispatch counter.
...
/// each op's `input[25]`/`output[25]` lanes into 25-stride interleaved
/// arrays plus per-row scalar arrays for timestamp/state_addr/flags.
pub fn try_generate_keccak_trace_gpu_raw(
    num_rows: usize,
    timestamps: &[u64],
    state_addrs: &[u64],
    inputs: &[u64],
    ...
    let raw = math_cuda::keccak_trace::generate_keccak_trace_dev(
        num_rows,
        timestamps,
        state_addrs,
        inputs,
    ...
    .ok()?;
    GPU_KECCAK_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}

/// Prover-crate wrapper for the GPU MEMW (general unaligned/split-ts path)
/// trace generator. Caller packs each op's `value[8]`/`old[8]`/
/// `old_timestamp[8]` into 8-stride interleaved arrays plus flat per-row
/// arrays for the scalar fields.
#[allow(clippy::too_many_arguments)]
pub fn try_generate_memw_trace_gpu_raw(
    num_rows: usize,
    base_addresses: &[u64],
    timestamps: &[u64],
    values: &[u64],
    ...
    let raw = math_cuda::memw_trace::generate_memw_trace_dev(
        num_rows,
        base_addresses,
        timestamps,
        values,
    ...
    .ok()?;
    GPU_MEMW_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}
...
/// each op's scalar inputs plus a bit-packed flags u64 (see the module
/// docs for the bit map); GPU computes the aux columns + row layout.
#[allow(clippy::too_many_arguments)]
pub fn try_generate_cpu32_trace_gpu_raw(
    num_rows: usize,
    timestamps: &[u64],
    pcs: &[u64],
    rv1s: &[u64],
    ...
    let raw = math_cuda::cpu32_trace::generate_cpu32_trace_dev(
        num_rows,
        timestamps,
        pcs,
        rv1s,
    ...
    .ok()?;
    GPU_CPU32_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}
...
/// deduped via HashMap (4-field key wider than u128); caller packs each
/// unique row's (lhs, rhs, flags, multiplicity).
pub fn try_generate_lt_trace_gpu_raw(
    num_rows: usize,
    lhs_values: &[u64],
    rhs_values: &[u64],
    flags: &[u64],
    ...
    let raw = math_cuda::lt_trace::generate_lt_trace_dev(
        num_rows,
        lhs_values,
        rhs_values,
        flags,
    ...
    .ok()?;
    GPU_LT_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}
...
/// each op's `value[8]`/`old[8]` into 8-stride interleaved arrays plus
/// flat per-row arrays for the scalar fields.
#[allow(clippy::too_many_arguments)]
pub fn try_generate_memw_aligned_trace_gpu_raw(
    num_rows: usize,
    base_addresses: &[u64],
    timestamps: &[u64],
    old_timestamps: &[u64],
    ...
    let raw = math_cuda::memw_aligned_trace::generate_memw_aligned_trace_dev(
        num_rows,
        base_addresses,
        timestamps,
        old_timestamps,
    ...
    .ok()?;
    GPU_MEMW_ALIGNED_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}

/// Prover-crate wrapper for the GPU SHIFT-trace generator. Caller packs each
/// row's `value`, `shift_amount`, and `flags` (bits 0..3 = direction|signed|
/// word_instr|active) into three parallel arrays of length `num_rows`.
pub fn try_generate_shift_trace_gpu_raw(
    num_rows: usize,
    in_values: &[u64],
    shift_amounts: &[u64],
    flags: &[u64],
    ...
    let raw = math_cuda::shift_trace::generate_shift_trace_dev(
        num_rows,
        in_values,
        shift_amounts,
        flags,
    ...
    .ok()?;
    GPU_SHIFT_TRACE_CALLS.fetch_add(1, Ordering::Relaxed);
    Some(raw)
}
...
/// Test-only: schedule the Nth upcoming `generate_store_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
...
/// Test-only: schedule the Nth upcoming `generate_shift_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
pub fn schedule_shift_trace_fault(n_calls_until_err: i64) {
    math_cuda::shift_trace::FAULT_SHIFT_TRACE_REMAINING_UNTIL_ERR
        .store(n_calls_until_err, Ordering::Relaxed);
}

/// Test-only: schedule the Nth upcoming `generate_memw_aligned_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
...
/// Test-only: schedule the Nth upcoming `generate_lt_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
...
/// Test-only: schedule the Nth upcoming `generate_cpu32_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
...
/// Test-only: schedule the Nth upcoming `generate_memw_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
pub fn schedule_memw_trace_fault(n_calls_until_err: i64) {
    math_cuda::memw_trace::FAULT_MEMW_TRACE_REMAINING_UNTIL_ERR
        .store(n_calls_until_err, Ordering::Relaxed);
}

/// Test-only: schedule the Nth upcoming `generate_keccak_trace_dev` call to Err.
#[cfg(feature = "test-cuda-faults")]
