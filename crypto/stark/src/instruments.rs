...
pub fn reset_all() {
    ...
    // NOTE: TRACE_BUILD_TIMINGS is NOT cleared here. It's populated during
    // the trace-build phase, which runs BEFORE `multi_prove::reset_all` is
    // called. Wiping it here would erase the data print_report needs to
    // show the per-table breakdown. `take_trace_build_table_timings` drains
    // the Mutex via `mem::take`, so cross-prove contamination in a long-
    // running process is impossible — each prove ends with the buffer at
    // length 0 after print_report consumes it.
}
