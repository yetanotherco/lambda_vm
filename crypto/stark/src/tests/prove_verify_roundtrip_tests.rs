...
/// Golden `(len, FNV-1a-64)` of the serialized demo multi-proof. LDE-internal
/// reworks (permute elimination, kernel changes) are proof-invariant, so this
/// must not move.
const LDE_PROOF_GOLDEN: (usize, u64) = (24321, 16457982168507117669);
...
fn test_verify_serialized_multi_table_proofs() {
    ...
    // LDE tripwire: LDE-internal reworks are proof-invariant, so the demo
    // proof must stay byte-identical (same FNV-1a digest). A mismatch flags
    // an index-map bug — verification alone can't catch a "wrong but
    // verifying" proof, this can.
    {
        ...
        assert_eq!(
            ...
            LDE_PROOF_GOLDEN,
            "LDE proof digest moved (len, fnv-1a)",
        );
    }
