...
use crate::prover::{Prover, IsStarkProver};
...
fn real_prep_root(air: &Air, trace: &TraceTable<F, E>, width: usize) -> crate::config::Commitment {
    ...
    let (data, total_cols) = Prover::<F, E, ()>::expand_main_lde_row_major(
        trace,
        &domain,
        &twiddles,
        #[cfg(feature = "disk-spill")]
    ...
    Prover::<F, E, ()>::commit_rows_bit_reversed_subset::<F>(
...
/// under both.
///
/// DEFERRED (M-6 preprocessed round): currently FAILS on this branch. The
/// batched prep opening commits the per-table precomputed tree at its own
/// depth (`height-1`), not the epoch's `h_max-1`, so an un-reduced iota that
/// collides on the low `height-1` bits authenticates. The preprocessed round
/// (M-6/M-7) is out of scope for the first CPU pass — the core batched path
/// (main/aux/composition + all its tamper negatives) is complete and green.
/// Fixing this requires porting the batched preprocessed-round binding so the
/// prep opening rides the epoch's index space. See PLAN-multi-merkle-tree.md.
#[ignore = "M-6 preprocessed-round index binding not yet ported — un-reduced index authenticates; preprocessed path is not soundness-validated"]
#[test_log::test]
...
mod carved {
    ...
    fn prove_carved() -> (Vec<Air>, Proof) {
        ...
        let pairs: Vec<_> = airs
            ...
            .collect();
        let (proof, _) = multi_prove_batched_carved::<F, E, (), P>(
            pairs,
            &mut DefaultTranscript::<E>::new(&[]),
            #[cfg(feature = "disk-spill")]
            crate::storage_mode::StorageMode::Ram,
    ...
    fn verifies_carved(airs: &[Air], proof: &Proof, carved: Option<usize>) -> bool {
        multi_verify_batched_carved::<F, E, (), V, _>(
            &refs(airs),
            proof,
            &mut DefaultTranscript::<E>::new(&[]),
            &FieldElement::zero(),
