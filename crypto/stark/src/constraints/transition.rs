    ...
    E: IsField + Send + Sync,
{
    ...
    fn evaluate_prover(
        ...
        self.evaluate_verifier(ctx, ext_evaluations);
    }
    ...
    E: IsField + Send + Sync,
{
    ...
    /// prover (F, E) and verifier (E, E) evaluation contexts.
    ...
    /// Wrap into a boxed `TransitionConstraintEvaluator` for the prover/verifier.
...
///
/// Auto-generates `evaluate_verifier()` and `evaluate_prover()` from the user's `evaluate()`.
    ...
    E: IsField + Send + Sync,
{
    ...
    fn evaluate_prover(
        ...
        if let TransitionEvaluationContext::Prover { frame, .. } = ctx {
            base_evaluations[self.0.constraint_idx()] =
                self.0.evaluate(frame.get_evaluation_step(0));
        } else {
