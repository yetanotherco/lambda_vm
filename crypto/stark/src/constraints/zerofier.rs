...
//!
//! These are the bodies of the `TransitionConstraintEvaluator` default
//! methods (`crate::constraints::transition`), relocated verbatim to consume
//! plain constraint metadata instead of trait getters — they only ever read
//! `period` / `offset` / `exemptions_period` / `periodic_exemptions_offset` /
//! `end_exemptions`. The engine's zerofier machinery moves onto these once
//! tables convert to [`ConstraintSet`](crate::constraints::builder::ConstraintSet);
//! until then the trait defaults remain the production path (equivalence is
...
// =============================================================================
// Equivalence tests: free functions == the trait defaults they were moved from
// =============================================================================
...
mod tests {
    ...
    /// A boxed-path constraint whose zerofier shape is configurable — used to
    /// run the ORIGINAL trait-default bodies for comparison.
    ...
    impl TransitionConstraintEvaluator<Fp, Ext> for DummyConstraint {
        ...
        fn evaluate_verifier(
            &self,
            _ctx: &TransitionEvaluationContext<Fp, Ext>,
            _evals: &mut [ExtE],
        ) {
        }
    ...
    impl DummyConstraint {
        fn meta(&self) -> ConstraintMeta {
            let mut m = ConstraintMeta::base(0, 1)
                .with_period(self.period)
                .with_offset(self.offset)
                .with_end_exemptions(self.end_exemptions);
            if let Some(p) = self.exemptions_period {
                m = m.with_exemptions(p, self.periodic_exemptions_offset.unwrap());
            }
            m
        }
    }

    /// Hand-built prover Domain: trace length 16, blowup 4, coset offset 7.
    fn sample_domain() -> Domain<Fp> {
        ...
        let trace_primitive_root =
            <Fp as math::field::traits::IsFFTField>::get_primitive_root_of_unity(root_order as u64)
                .unwrap();
    ...
    /// The zerofier configurations exercised: every branch of the moved
    /// bodies (default; end exemptions; period/offset; periodic exemptions
    /// with and without end exemptions).
    fn sample_configs() -> Vec<DummyConstraint> {
        vec![
            // Every row, no exemptions (the common case).
            DummyConstraint {
                period: 1,
                offset: 0,
            ...
            // Periodic constraint with offset.
            DummyConstraint {
                period: 2,
                offset: 1,
                exemptions_period: None,
            ...
            // Periodic constraint with offset and end exemptions.
            DummyConstraint {
                period: 4,
                offset: 3,
                exemptions_period: None,
            ...
            // Periodic exemptions (bit_flags-shaped), cyclic fast path.
            DummyConstraint {
                period: 1,
                offset: 0,
                exemptions_period: Some(4),
                periodic_exemptions_offset: Some(3),
                end_exemptions: 0,
            },
            // Periodic exemptions + end exemptions.
            DummyConstraint {
                period: 1,
                offset: 0,
                exemptions_period: Some(4),
                periodic_exemptions_offset: Some(1),
                end_exemptions: 2,
            },
        ]
    }
    ...
    #[test]
    fn lde_zerofier_evaluations_match_trait_defaults() {
        let domain = sample_domain();
        for (i, c) in sample_configs().iter().enumerate() {
            let expected = c.zerofier_evaluations_on_extended_domain(&domain);
            let got = zerofier_evaluations_on_extended_domain(&c.meta(), &domain);
            assert_eq!(got, expected, "config {i} mismatch");
        }
    }

    #[test]
    fn ood_zerofier_evaluations_match_trait_defaults() {
        let domain = sample_domain();
        let trace_length = domain.trace_roots_of_unity.len();
        let root = &domain.trace_primitive_root;
        // A few arbitrary OOD points with all three extension components set.
        let zs = [
            ExtE::from_raw([FpE::from(123u64), FpE::from(456u64), FpE::from(789u64)]),
            ExtE::from_raw([
                FpE::from(0xDEAD_BEEFu64),
                FpE::from(0xCAFE_F00Du64),
                FpE::from(0x1234_5678u64),
            ]),
        ];
        for (i, c) in sample_configs().iter().enumerate() {
            for z in &zs {
                let expected = c.evaluate_zerofier(z, root, trace_length);
                let got = evaluate_zerofier(&c.meta(), z, root, trace_length);
                assert_eq!(got, expected, "config {i} mismatch at z={z:?}");
            }
        }
    }

    #[test]
    fn end_exemptions_helpers_match_trait_defaults() {
        let domain = sample_domain();
        let trace_length = domain.trace_roots_of_unity.len();
        let root = &domain.trace_primitive_root;
        for (i, c) in sample_configs().iter().enumerate() {
            assert_eq!(
                end_exemptions_roots(&c.meta(), root, trace_length),
                c.end_exemptions_roots(root, trace_length),
                "config {i} roots mismatch"
            );
            assert_eq!(
                end_exemptions_lde_evaluations(&c.meta(), &domain),
                c.end_exemptions_lde_evaluations(&domain),
                "config {i} lde evaluations mismatch"
            );
        }
    }
}
