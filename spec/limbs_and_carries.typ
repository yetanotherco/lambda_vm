...
$
  ...
  &= sum_(r=0)^(2(n-1)) dash(w)_r dot L^r, #<limbs:eq:f-semi-decomposition>
$
where
$
  dash(w)_i := sum_(m in [mu]) (sum_(j=0)^(i) vec(x)^((m))_(i-j) dot vec(y)^((m))_j) + sum_(a in [alpha]) vec(z)^((a))_i.
$

While @limbs:eq:f-semi-decomposition closely resembles that of a limb-decomposition (@limbs:eq:decomposition), 
there is the problem that $dash(w)_i$ will generally not be bounded by $L$.
We therefore introduce a helper sequence, $c_i$, to transform $dash(w)_i$ into a proper decomposition $w_i$ as:
$
  w_i &:= dash(w)_i + c_(i-1) mod L &text("for") i >= 0,\
  c_i &:= (dash(w)_i + c_(i-1) - w_i )/L &text("for") i >= 0,\
$
...
#proof[
    Reordering the definition of $c_i$, we find the equality $w_i = dash(w)_i + c_(i-1) - c_i dot L$.
    Leveraging this, we see that
    $  
    sum_(r=0)^(g-1) w_r dot L^r
    &= sum_(r=0)^(g-1) (dash(w)_r + c_(r-1) - c_r dot L) dot L^r\
    &= (sum_(r=0)^(g-1) dash(w)_r dot L^r) + (sum_(s=0)^(g-1) c_(s-1) dot L^s) - (sum_(t=0)^(g-1) c_t dot L^(t+1))\
    &= (sum_(r=0)^(g-1) dash(w)_r dot L^r) + (sum_(s=-1)^(g-2) c_(s) dot L^(s+1)) - (sum_(t=0)^(g-1) c_t dot L^(t+1))\
    &= (sum_(r=0)^(g-1) dash(w)_r dot L^r) + c_(-1) - c_(g-1) dot L^(g-1+1)\
    &= (sum_(r=0)^(g-1) dash(w)_r dot L^r) - c_(g-1) dot L^(g),\
    &= (sum_(r=0)^(2(n-1)) dash(w)_r dot L^r) - c_(g-1) dot L^(g),\
    &= f_(mu, alpha)(vec(x), vec(y), vec(z)) - c_(g-1) dot L^(g),\
    $
    where the second-to-last step follows from the observation that $dash(w)_j = 0$ for $j > 2(n-1)$.
...
#proof[
  Since $w_i in [L]$,
  $c_i 
  := frac((dash(w)_i + c_(i-1) - w_i ), L, style: "horizontal")
  = floor.l frac((dash(w)_i + c_(i-1)), L, style: "horizontal") floor.r.$
  Hence, $c_i$ is maximized when both $dash(w)_i$ and $c_(i-1)$ are, and thus, 
  by induction, when $dash(w)_j$ is maximized for all $j <= i$.
...
#proof[
  ...
  $
  c_(n)
  &= lr(floor.l (dash(w)_n + c_(n-1))/ L floor.r)\
...
Lastly, we prove that there exists a correct method of constraining the relation between $dash(w)_i$, $w_i$ and $c_i$ inside this VM:

#lemma("Constraint correctness")[
    Let $L >= 4$, and $c_i, w_i in FF_p$ with $p$ prime. 
    The constraints
    $
        c_i &= (dash(w)_i + c_(i-1) - w_i) dot L^(-1), #<limbs:eq:def_ci>\
        c_i &in [C], #<limbs:eq:range_ci>\
        c_(-1) &= 0, #<limbs:eq:c_-1_is_zero>\
        w_i &in [L] #<limbs:eq:range_wi>
    $
    together enforce $w_i = dash(w)_i + c_(i-1) mod L$ as long as $C in [mu n L + alpha, frac(p,L, style:"horizontal"))$.
...
#proof[
    ...
    $
    &&c_i &in [C]\
    &<=>& dash(w)_i + c_(i-1) - w_i &in {0, L, ..., (C-1)L},\
    &<=>& w_i &in {dash(w)_i + c_(i-1), dash(w)_i + c_(i-1) - L, ..., dash(w)_i + c_(i-1) - (C-1)L}.
    $
    Let us use $X_(i)$ to refer to this last set.
    Now --- under the assumption that $c_(i-1)$ is correct --- 
    observe that $w_i := dash(w)_i + c_(i-1) mod L in X_i$, since
    $
        dash(w)_i + c_(i-1)
    ...
    $dash(w)_i <= mu n(L-1)^2 + alpha (L-1)$ 
    ...
    Constraint @limbs:eq:range_wi therefore enforces that $w_i = dash(w)_i + c_(i-1) mod L$ if $c_(i-1)$ (and therefore $w_(i-1)$) is correct, 
    and as a result, $c_i = floor.l frac((dash(w)_i + c_(i-1)), L, style: "horizontal") floor.r$ is correct.
