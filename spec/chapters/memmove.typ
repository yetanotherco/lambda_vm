...
The #memmove chip copies a range of bytes from one location in memory to another, that is, it performs a `memmove`.
#footnote([Linux man-page on `memmove`; man7.org. #link("https://man7.org/linux/man-pages/man3/memmove.3.html")[[src]]])
...
every later row receives all four over the `MEMMOVE_NEXT` bus, where @memmove:c:range_src_incr, @memmove:c:range_dst_incr and @memmove:c:range_count_decr range-check them on the sending side.

= Constraints
In this VM, we assign system call number $-30$ to the #memmove accelerator.
...
@memmove:c:read_dst, @memmove:c:read_src and @memmove:c:read_count read these three registers.
...
@memmove:c:tail pins that choice to `LT` (@lt): the prover cannot select a convenient partition of the requested range.
...
@memmove:c:bound therefore proves $#`count` < 257$ on the first row of every sequence, which caps a sequence at $39$ rows --- attained at $n = 255$, not at $n = 256$, since a byte short of the bound trades one eight-byte row for seven one-byte rows.
...
The `CPU`'s preprocessed timestamp column holds $4 dot (i + 1)$ at row $i$ (@vars), so $#`timestamp` + 2$ cannot leave the `Word` range in @memmove:c:write_value --- `IS_WORD` on `timestamp` alone would not rule that out.
...
And since @memmove:c:tail picks the width from `count` alone, an end that begins misaligned stays misaligned for every chunk of the sequence --- this chip never emits a short leading row to realign one.
A misaligned copy therefore commits strictly more cells than an aligned copy of the same length while producing exactly the same number of #memmove rows, which is the reason the row counts this chapter reasons about are not on their own the cost of a copy.
...
+ The addresses use `ADDNW` (@add), which forbids wraparound modulo $2^64$.
  ...
  The condition is $#`μ` - #`end`$: on the terminal row and on padding rows the computed successor is consumed by nobody, since @memmove:c:send_copy_next_chunk carries that same multiplicity.
+ The count uses plain `SUB` (@add), which permits wraparound, because the terminal row holds $#`count` = 0$ and hence $#`count_decr` = 0 - 1 = 2^64 - 1$.
  That permission is safe precisely because @memmove:c:tail pins $#`tail` = (#`count` < 8)$, so $#`step` <= #`count`$ on every row with $#`count` >= 1$ and the subtraction can only wrap on the terminal row.
...
== Chaining the rows
When this was not the last chunk of this sequence, we recursively copy the next chunk over the `MEMMOVE_NEXT` bus, specifying the timestamp, the addresses to continue reading and writing at, and the number of bytes that still have to be copied (@memmove:c:send_copy_next_chunk).
...
Since the CPU's timestamps strictly increase per instruction, no two #memmove `ECALL`s share one.
...
Fix a timestamp. Balancing `MEMMOVE_NEXT` forces the number of rows claiming `end` to equal the number claiming `first`, and @memmove:c:receive_ecall caps the latter at one, since the `CPU` sends a single `ECALL` per timestamp.
...
What forbids the ring is @memmove:c:src_incr: `ADDNW` forces $#`src_incr` = #`src` + #`step`$ _over the integers_ with $#`step` >= 1$, so `src` strictly increases along the chain and can never return to a value it already held.
This is the second reason the addresses use `ADDNW` rather than `ADD`, and the more important of the two.

== Bits
Lastly, we must make sure `first`, `end`, `tail` and `μ` are bits, and that either $#`first` = 1$ or $#`end` = 1$ implies $#`μ` = 1$ (@memmove:c:first_or_end_implies_mu).
...
It is not the only one --- @memmove:c:range_src_incr and @memmove:c:range_dst_incr, which would pin the limbs, carry multiplicity $#`μ`$ and are inert here --- but a padding row feeds no interaction either way.
...
@memmove:c:tail meets that by choosing each row's width from `count` alone, so no alignment of `dst`, `src` or `count` is assumed.
...
- The `memmove` property belongs to one `ECALL`, not to an arbitrarily large guest-level copy: a copy larger than 256 bytes is split by the guest stub into several `ECALL`s at distinct timestamps, and chunk $k+1$ reads what chunk $k$ has already written.
  That is in-contract for `memcpy`, whose buffers may not overlap; a guest-side `memmove` would have to keep each copy within a single `ECALL`, or chunk in the direction that preserves the semantics.
- The `value` variable is typed as bytes, but this chip range-checks none of its lanes.
  On a row that copies, they are pinned by @memmove:c:read_value instead: a lane holds whatever the memory argument says resides at that address.
  ...
  That is harmless, because @memmove:c:write_value carries the same multiplicity and so does not fire either.
...
- `count` need not be a full `DWordWL`: @memmove:c:bound already proves $#`count` < 257$ on the first row of a sequence, and every later `count` is smaller still.
