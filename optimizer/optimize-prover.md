...
> **2026-05-15 — SCOPE EXPANDED FOR THIS WORKSTREAM**: protocol-level changes
> ARE allowed. After Attempt 1 (`experiment/eval-d1-domain-n`) and Attempt 2
> (`experiment/batched-lin-combination`) ran out of headroom within the
> original "transparent-to-verifier" scope, the user explicitly opened the
> protocol to modification. This is documented in
> `project_optimizer_protocol_scope` memory. Default optimizer behavior
> remains restrictive in other contexts.

**Default rule (relaxed for this workstream)**: optimizations should aim to
preserve verifier behavior when possible, but **may** modify the protocol if
benchmarking shows that's the only way to close the structural gap.

### ALLOWED (prover infrastructure, no protocol change)
...
- `crypto/stark/src/fri/` — commitment phase, query phase, evaluation order (algorithmic, same protocol)
...
### ALSO ALLOWED IN THIS WORKSTREAM (protocol-level)

- `crypto/stark/src/verifier.rs` — can change if the prover-side change
  requires it. Must update tests + soundness argument.
- `crypto/stark/src/proof/` — proof struct shape can evolve (e.g.,
  `composition_poly_root: Commitment` → `quotient_chunk_roots: Vec<Commitment>`).
- FRI structure changes (e.g., FRI over multiple commitments instead of one).
- Round structure / commit shape / Fiat-Shamir transcript order — can be
  modified as long as new prover and new verifier agree.

### STILL FORBIDDEN

- AIR/Constraint definitions of existing AIRs (Keccak, CPU, etc.) — those
  are about correctness of the executed program, not the prover architecture.
- Changes that break soundness without a soundness argument or test coverage.

### Verification under protocol changes

When a change touches the protocol, the baseline verifier
(`/tmp/cli_baseline_verifier`) is **no longer valid as oracle** — its proof
format doesn't match the new one. Instead:

1. Build BOTH `cli` (new prover + new verifier) on the experiment branch.
2. Produce proof: `target/release/cli prove <ELF>`.
3. Verify with the **same branch's** verifier: `target/release/cli verify <proof> <ELF>`.
4. Document the soundness argument in the attempt's report entry.
5. Cross-check: previous proofs (generated by `bench_vs_p3` HEAD's cli) MUST
   still verify with their original verifier — no regression on existing
   protocol while we explore. Keep `/tmp/cli_baseline_verifier` for that
   sanity check.
