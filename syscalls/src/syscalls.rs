...
/// Syscall number for the full-point ECSM variant (full point in/out).
/// Must match `executor::...::execution::ECSM_FULL_POINT_SYSCALL_NUMBER` (u64::MAX - 11).
#[cfg(target_arch = "riscv64")]
const ECSM_FULL_POINT_SYSCALL_NUMBER: usize = usize::MAX - 11;
...
/// FULL POINT: compute `k·(xG, yG)` on secp256k1 and write BOTH result coordinates into a
...
#[cfg(target_arch = "riscv64")]
pub fn ecsm_mul_full_point(out: &mut [u8; 64], input: &[u8; 64], k: &[u8; 32]) {
    unsafe {
        asm!(
            "ecall",
            ...
            in("a2") k.as_ptr(),        // x12 = address of k
            in("a7") ECSM_FULL_POINT_SYSCALL_NUMBER,
        )
    }
}

#[cfg(not(target_arch = "riscv64"))]
pub fn ecsm_mul_full_point(_out: &mut [u8; 64], _input: &[u8; 64], _k: &[u8; 32]) {
    unimplemented!("syscalls are only implemented for riscv64 targets");
}
