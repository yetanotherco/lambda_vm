...
#[test]
fn test_empty_logs() {
    let result = Traces::from_logs(&[], U64HashMap::default(), 0x1000, &Default::default());
...
fn test_single_log() {
    ...
    let _traces = Traces::from_logs(&logs, instructions, 0x1000, &Default::default()).unwrap();
}
...
fn test_power_of_two_logs() {
    ...
    let traces = Traces::from_logs(&logs, instructions, 0x1000, &Default::default()).unwrap();
