...
fn dump_measurement_blobs() {
    ...
    // preprocessed PAGE genesis, so a wrong root fails `multi_verify` -> the guest
    // rejects (Ok(None) -> panic on the guest). Demonstrates execute-only that the
    // supplied commitments are load-bearing on the v2 path (the host
