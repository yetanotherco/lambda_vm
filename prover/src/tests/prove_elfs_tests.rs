...
fn test_prove_elfs_test_sb_sh_8() {
    ...
    // LOAD sends to Memw bus with multiplicity MU
    let mut load_memw_sends: u64 = 0;
    for row in 0..traces.load.num_rows() {
        let mu = traces.load.main_table.get(row, load_cols::MU).to_raw();
        load_memw_sends += mu;
    }

    // MEMW receives from Memw bus with MU_READ + MU_WRITE
    ...
    // Break down by READ vs WRITE signature
    ...
    // MEMW read/write receives
    let mut memw_read_receives: u64 = 0;
...
fn test_single_page_table_balance() {
    ...
    // Run the prover
    let result = prove_and_verify_vm_with_pages(
        &mut traces.cpu,
        &mut traces.bitwise,
        &mut traces.lt,
        &mut traces.memw,
