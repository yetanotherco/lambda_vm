...
///
/// Heavy (~4.4M executed instructions, ~54s to prove+verify), so ignored for
/// time like the empty-block sibling — but it PASSES.
///
/// History: this previously failed verification with "LogUp bus does not
/// balance". Root cause (found via `debug_ethrex_simple_tx_bus_balance`): the
/// `Print` syscall (a7=1) was emitted on the Ecall bus by CPU with no receiver
/// table, and the tx path calls `sys_rand` -> `print_string` -> Print ecalls,
/// leaving orphan senders. Fixed by making `print_string` a no-op
/// (syscalls/src/syscalls.rs) so no guest path emits a Print ecall.
/// Run with `cargo test -p lambda-vm-prover --release --lib \
/// test_prove_ethrex_simple_tx -- --ignored --nocapture`.
#[test]
#[ignore = "takes too long (~54s; ~4.4M instructions)"]
...
/// Diagnostic for the `test_prove_ethrex_simple_tx` LogUp imbalance.
///
/// Builds the traces (cheap — no LDE/FRI/proving, so it fits in memory where the
/// full debug-checks prover OOMs on a 4.4M-row trace) and checks bus balance
/// directly at the trace level:
///   Pass 1 — per-bus signed multiplicity totals (sender +, receiver −). A
///     nonzero net means that bus has unmatched (orphan) interactions → the
///     culprit bus. Buses 20/21 (CommitNextByte/Commit) net nonzero by design
///     (external verifier-computed receiver), so they're reported but expected.
///   Pass 2 — for each flagged bus, per-tuple residuals to surface the exact
///     orphan value-tuples and which tables send/receive them.
/// Run: `cargo test -p lambda-vm-prover --release --lib \
///   debug_ethrex_simple_tx_bus_balance -- --ignored --nocapture`
#[test]
#[ignore = "diagnostic for the simple-tx LogUp imbalance (trace-level, no proving)"]
fn debug_ethrex_simple_tx_bus_balance() {
    use crate::tables::trace_builder::Traces;
    use crate::tables::{
        bitwise, branch, commit, cpu, decode, dvrm, halt, keccak, keccak_rc, keccak_rnd, load, lt,
        memw, memw_aligned, memw_register, mul, page, register, shift,
    };
    use std::collections::HashMap;
    use stark::lookup::BusInteraction;

    type Tt = stark::trace::TraceTable<GoldilocksField, GoldilocksExtension>;
    const P: i128 = 18_446_744_069_414_584_321; // Goldilocks prime 2^64 - 2^32 + 1
    let to_signed = |v: u64| -> i128 {
        let v = v as i128;
        if v > P / 2 { v - P } else { v }
    };
    let bus_name = |id: u64| -> &'static str {
        match id {
            0 => "AreBytes", 1 => "IsHalfword", 2 => "IsB20", 3 => "AndByte", 4 => "OrByte",
            5 => "XorByte", 6 => "Msb8", 7 => "Msb16", 8 => "Zero", 9 => "Hwsl", 10 => "Lt",
            11 => "Mul", 12 => "Dvrm", 13 => "Shift", 14 => "Memw", 15 => "Load", 16 => "Memory",
            17 => "Branch", 18 => "Decode", 19 => "Ecall", 20 => "CommitNextByte", 21 => "Commit",
            22 => "Keccak", 23 => "KeccakRc", _ => "?",
        }
    };
    ...
    let program = Elf::load(&elf_bytes).expect("load elf");
    let result = Executor::new(&program, input.clone())
        .expect("executor")
        .run()
        .expect("run");
    let traces =
        Traces::from_elf_and_logs(&program, &result.logs, &MaxRowsConfig::default(), &input)
            .expect("traces");

    // (label, traces, interactions) for every table except PAGE (handled below).
    let tables: Vec<(&str, Vec<&Tt>, Vec<BusInteraction>)> = vec![
        ("BITWISE", vec![&traces.bitwise], bitwise::bus_interactions()),
        ("DECODE", vec![&traces.decode], decode::bus_interactions()),
        ("REGISTER", vec![&traces.register], register::bus_interactions()),
        ("HALT", vec![&traces.halt], halt::bus_interactions()),
        ("COMMIT", vec![&traces.commit], commit::bus_interactions()),
        ("KECCAK", vec![&traces.keccak], keccak::bus_interactions()),
        ("KECCAK_RND", vec![&traces.keccak_rnd], keccak_rnd::bus_interactions()),
        ("KECCAK_RC", vec![&traces.keccak_rc], keccak_rc::bus_interactions()),
        ("CPU", traces.cpus.iter().collect(), cpu::bus_interactions()),
        ("LT", traces.lts.iter().collect(), lt::bus_interactions()),
        ("SHIFT", traces.shifts.iter().collect(), shift::bus_interactions()),
        ("MEMW", traces.memws.iter().collect(), memw::bus_interactions()),
        ("MEMW_A", traces.memw_aligneds.iter().collect(), memw_aligned::bus_interactions()),
        ("MEMW_R", traces.memw_registers.iter().collect(), memw_register::bus_interactions()),
        ("LOAD", traces.loads.iter().collect(), load::bus_interactions()),
        ("MUL", traces.muls.iter().collect(), mul::bus_interactions()),
        ("DVRM", traces.dvrms.iter().collect(), dvrm::bus_interactions()),
        ("BRANCH", traces.branches.iter().collect(), branch::bus_interactions()),
    ];
    // PAGE: one trace per page, interactions parameterized by page_base.
    let pages: Vec<(Vec<&Tt>, Vec<BusInteraction>)> = traces
        .pages
        .iter()
        .zip(traces.page_configs.iter())
        .map(|(t, cfg)| (vec![t], page::bus_interactions(cfg.page_base)))
        .collect();

    // ---- Pass 1: per-bus signed multiplicity totals, split by table ----
    // bus_id -> net, and (bus_id, table) -> net (for attribution).
    let mut bus_net: HashMap<u64, i128> = HashMap::new();
    let mut by_table: HashMap<(u64, String), i128> = HashMap::new();
    let mut accumulate = |label: &str, tts: &[&Tt], inters: &[BusInteraction]| {
        for tt in tts {
            let cols = tt.columns_main();
            let n = tt.num_rows();
            for inter in inters {
                let sign: i128 = if inter.is_sender { 1 } else { -1 };
                let mut net = 0i128;
                for row in 0..n {
                    let m = to_signed(inter.multiplicity.evaluate_at_row(&cols, row).canonical());
                    net += m;
                }
                *bus_net.entry(inter.bus_id).or_default() += sign * net;
                *by_table.entry((inter.bus_id, label.to_string())).or_default() += sign * net;
            }
        }
    };
    for (label, tts, inters) in &tables {
        accumulate(label, tts, inters);
    }
    for (tts, inters) in &pages {
        accumulate("PAGE", tts, inters);
    }

    println!("\n=== TRACE-LEVEL BUS BALANCE (signed multiplicity totals) ===");
    let mut ids: Vec<u64> = bus_net.keys().copied().collect();
    ids.sort();
    let mut flagged: Vec<u64> = vec![];
    for id in ids {
        let net = bus_net[&id];
        let tag = match id {
            20 | 21 => "(external — expected nonzero)",
            _ if net != 0 => "<<< IMBALANCED",
            _ => "",
        };
        println!("Bus {:2} {:14} net = {:>20} {}", id, bus_name(id), net, tag);
        if net != 0 && id != 20 && id != 21 {
            flagged.push(id);
            for ((bid, table), v) in by_table.iter() {
                if *bid == id && *v != 0 {
                    println!("        {:10} contributes {:>20}", table, v);
                }
            }
        }
    }
    ...
    if flagged.is_empty() {
        println!(
            "\nNo count-imbalance found. The bug is a value-mismatch with equal counts; \
             rerun pass 2 logic over all buses (per-tuple) to localize."
        );
        return;
    }

    // ---- Pass 2: per-tuple residuals for each flagged bus ----
    for &target in &flagged {
        println!("\n=== ORPHAN TUPLES on bus {} ({}) ===", target, bus_name(target));
        let mut tuples: HashMap<Vec<u64>, i128> = HashMap::new();
        let mut senders: HashMap<Vec<u64>, String> = HashMap::new();
        let mut receivers: HashMap<Vec<u64>, String> = HashMap::new();
        const CAP: usize = 3_000_000;
        let mut capped = false;
        let mut scan = |label: &str, tts: &[&Tt], inters: &[BusInteraction]| {
            let bus_inters: Vec<&BusInteraction> =
                inters.iter().filter(|i| i.bus_id == target).collect();
            if bus_inters.is_empty() {
                return;
            }
            for tt in tts {
                let cols = tt.columns_main();
                let n = tt.num_rows();
                for inter in &bus_inters {
                    let sign: i128 = if inter.is_sender { 1 } else { -1 };
                    for row in 0..n {
                        let m =
                            to_signed(inter.multiplicity.evaluate_at_row(&cols, row).canonical());
                        if m == 0 {
                            continue;
                        }
                        let mut key = Vec::new();
                        for bv in &inter.values {
                            for el in bv.combine_from(|c| cols[c][row].clone()) {
                                key.push(el.canonical());
                            }
                        }
                        if !tuples.contains_key(&key) && tuples.len() >= CAP {
                            capped = true;
                            continue;
                        }
                        *tuples.entry(key.clone()).or_default() += sign * m;
                        if inter.is_sender {
                            senders.entry(key).or_insert_with(|| label.to_string());
                        } else {
                            receivers.entry(key).or_insert_with(|| label.to_string());
                        }
                    }
                }
            }
        ...
        for (label, tts, inters) in &tables {
            scan(label, tts, inters);
        }
        for (tts, inters) in &pages {
            scan("PAGE", tts, inters);
        }
        if capped {
            println!("  (WARNING: tuple map hit cap {CAP}; report may be partial)");
        }
        let mut orphans: Vec<(&Vec<u64>, i128)> =
            tuples.iter().filter(|(_, v)| **v != 0).map(|(k, v)| (k, *v)).collect();
        orphans.sort_by_key(|(_, v)| -(v.abs()));
        println!("  {} distinct orphan tuple(s):", orphans.len());
        for (key, v) in orphans.iter().take(25) {
            let side = if *v > 0 { "over-SENT" } else { "over-RECV" };
            let s = senders.get(*key).map(|s| s.as_str()).unwrap_or("-");
            let r = receivers.get(*key).map(|s| s.as_str()).unwrap_or("-");
            println!("    residual {:>12} [{}]  sender={} receiver={}  tuple={:?}", v, side, s, r, key);
        }
        if orphans.len() > 25 {
            println!("    ... and {} more", orphans.len() - 25);
        }
    }
}
