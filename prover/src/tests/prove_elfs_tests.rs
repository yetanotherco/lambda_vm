...
/// ASM test: reads private input, does LD/SD/LB/SB on that data.
/// Tests that non-commit instructions can read the GPI-written bytes.
#[test]
fn test_prove_private_input_loads() {
    let _ = env_logger::builder().is_test(true).try_init();
    let elf_bytes = crate::test_utils::asm_elf_bytes("test_private_input_large");
    let input: Vec<u8> = (0..16).collect(); // 16 bytes: 0..15
