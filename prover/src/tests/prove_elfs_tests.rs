...
/// Tests private input → prover pipeline in seconds.
#[test]
fn test_prove_private_input_commit_asm() {
    let _ = env_logger::builder().is_test(true).try_init();
    let elf_bytes = crate::test_utils::asm_elf_bytes("test_private_input_commit");
    let input = vec![0xAA, 0xBB, 0xCC, 0xDD];
    let vm_proof = crate::prove_with_input(&elf_bytes, input.clone()).expect("Proving failed");
    assert!(
        crate::verify(&vm_proof, &elf_bytes).expect("Verification failed"),
        "Private input + commit ASM proof should verify"
    );
    // TODO: public_output is [0,0,0,0] because GetPrivateInputs writes are not yet
    // tracked in the prover's MemoryState. The proof is valid but proves wrong output.
    // Fix: generate MEMW operations for GetPrivateInputs byte writes.
    assert_eq!(vm_proof.public_output.len(), 4, "Output should be 4 bytes");
}

/// Minimal test: prove a program that reads private input and commits it.
/// This isolates whether private inputs work with the prover.
#[test]
fn test_prove_private_input_commit() {
    let _ = env_logger::builder().is_test(true).try_init();
    let workspace_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("workspace root")
        .to_path_buf();
    let elf_bytes = std::fs::read(workspace_root.join("executor/program_artifacts/rust/commit.elf"))
        .expect("Failed to read commit.elf");
    let input = vec![0xAA, 0xBB, 0xCC, 0xDD];
    let vm_proof = crate::prove_with_input(&elf_bytes, input.clone()).expect("Proving failed");
    assert!(
        crate::verify(&vm_proof, &elf_bytes).expect("Verification failed"),
        "Private input + commit proof should verify"
    );
