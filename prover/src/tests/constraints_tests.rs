...
fn test_create_all_cpu_constraints() {
    ...
    assert_eq!(add.len(), 8);
    // Other: branch_cond(1) + ebreak(1) + rv1_zero_forcing(3) + rv2_zero_forcing(3) + arg1(2) + arg2(2) + rvd(2) + slt_zero(7) + sign_bit_zero(1) + ecall_commit_implies_ecall(1) + ecall_keccak_implies_ecall(1) + next_pc(2) = 26
