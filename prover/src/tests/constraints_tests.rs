...
fn test_cpu_bit_flag_columns_count() {
    // Should have 35 bit flag columns (includes read_register1, read_register2, ecall_keccak, inline-pc columns)
    assert_eq!(BIT_FLAG_COLUMNS.len(), 35);
}
...
fn test_create_all_cpu_constraints() {
    ...
    assert_eq!(is_bit.len(), 35);
    // ADD constraints: 2 (ADD+LOAD) + 2 (STORE: arg1+imm) + 2 (SUB+BEQ) + 2 (JALR) = 8
    assert_eq!(add.len(), 8);
    // Other: branch_cond(1) + ebreak(1) + ecall_keccak_implies(1) + rv1_zero_forcing(3) + rv2_zero_forcing(3) + arg1(2) + arg2(2) + rvd(2) + slt_zero(7) + ext_bit_zero(3) + next_pc(2) = 27
    assert_eq!(other.len(), 27);

    // Total should be 35 + 8 + 27 = 70
    assert_eq!(total, 70);
