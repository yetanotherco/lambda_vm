...
fn test_cpu_bit_flag_columns_count() {
    // Should have 34 bit flag columns (includes read_register1, read_register2, ECALL_COMMIT, ECALL_KECCAK)
...
fn test_create_all_cpu_constraints() {
    ...
    assert_eq!(add.len(), 8);
    // Other: branch_cond(1) + ebreak(1) + rv1_zero_forcing(3) + rv2_zero_forcing(3) + arg1(2) + arg2(2) + rvd(2) + slt_zero(7) + ext_bit_zero(3) + ecall_commit_implies_ecall(1) + ecall_keccak_implies_ecall(1) + next_pc(2) = 28
    assert_eq!(other.len(), 28);

    // Total should be 34 + 8 + 28 = 70
    assert_eq!(total, 70);
