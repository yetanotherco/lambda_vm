...
///
/// ⚠ Under `one_row=auto` (the provisional default, RULINGS 26) every case
/// here resolves to ROW PAIRS — these AIRs are narrow and short, where the
/// auto rule keeps pairs — so these bytes did not move when `one_row` flipped
/// from 0 to auto (asserted in `production_format_rpx_goldens_are_byte_identical`).
/// One-row proof bytes are pinned by the (e) vectors (`tests::zf_rpx_vectors`,
/// `stark::tests::zf_fri_vectors`) and the VM bytes device comparison.
