...
///
/// ⚠ Every case here resolves to ROW PAIRS under the default's `one_row=auto`
/// (asserted in `production_format_rpx_goldens_are_byte_identical`), so these
/// bytes are also the `one_row=0` bytes. One-row proof bytes are pinned by the
/// (e) vectors (`tests::zf_rpx_vectors`, `stark::tests::zf_fri_vectors`).
...
fn production_format_rpx_goldens_are_byte_identical() {
    ...
    // What the pins cover: every case at row pairs, whatever `one_row` says.
    for (rows, blowup) in [(16usize, 2u8), (64, 4)] {
        let o = options(blowup, 2, 5, f);
        let air = SimpleAdditionAIR::<F>::new(&o);
        assert!(
            !stark::leaf_layout::table_leaf_layout(&air, rows).is_one_row(),
            "simple_addition rows {rows}: the production pins assume row pairs"
        );
    }
    for (rows, blowup) in [(32usize, 4u8), (128, 2)] {
        let o = options(blowup, 1, 7, f);
        let air = LogReadOnlyRAP::<F, E>::new(&o);
        assert!(
            !stark::leaf_layout::table_leaf_layout(&air, rows).is_one_row(),
            "logup rows {rows}: the production pins assume row pairs"
        );
    }
...
/// (`ZfFormat::DEFAULT`: cap auto, `fri=dp` group layers, `one_row=auto`); the
/// knobs select the arms — `LAMBDA_VM_ZF_FRI=pair` (legacy pair layers),
/// `LAMBDA_VM_ZF_ONE_ROW=0|1|auto` (both sites stamp the one-row mode; a table
