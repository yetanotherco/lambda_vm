...
/// [`verify_and_attest_blob`]'s core for a continuation bundle (the
/// `continuation` guest feature): verify every epoch + the global memory
/// proof against the supplied roots, then attest
/// `program_id(elf, roots) || public_output`. The fold uses
...
/// [`verify_continuation_and_attest`] over the wire-format blob
/// ([`encode_continuation_guest_input`]) — the `continuation` guest's whole
/// job in one call. The archive is bytecheck-validated, then the input is
/// materialized with one `rkyv::deserialize` pass (a structured copy, not a
/// parse) because the epoch/global verifiers currently consume an owned
/// [`crate::continuation::ContinuationProof`]; verifying epochs zero-copy over
/// the archived bundle (as [`crate::verify_recursion_blob`] does for the
/// monolithic proof) is follow-up work.
pub fn verify_continuation_and_attest_blob(
    ...
    let input: ContinuationGuestInput = rkyv::deserialize::<_, RkyvError>(archived)
        .map_err(|e| Error::Execution(format!("continuation blob deserialize failed: {e}")))?;
