...
// =========================================================================
// Helper functions for constraint computation
// =========================================================================

/// Convert lhs (DWordHHW) to DWordWL format.
/// DWordHHW: [Word, Half, Half] -> DWordWL: [Word, Word]
/// lo = lhs[0]
/// hi = lhs[1] + 2^16 * lhs[2]
#[inline]
pub fn dwordhhw_to_dwordwl(word: u32, half1: u16, half2: u16) -> (u32, u32) {
    let lo = word;
    let hi = (half1 as u32) + ((half2 as u32) << 16);
    (lo, hi)
}

/// Convert DWordHL to DWordWL format.
/// DWordHL: [Half, Half, Half, Half] -> DWordWL: [Word, Word]
/// lo = half[0] + 2^16 * half[1]
/// hi = half[2] + 2^16 * half[3]
#[inline]
pub fn dwordhl_to_dwordwl(h0: u16, h1: u16, h2: u16, h3: u16) -> (u32, u32) {
    let lo = (h0 as u32) + ((h1 as u32) << 16);
    let hi = (h2 as u32) + ((h3 as u32) << 16);
    (lo, hi)
}
...
mod tests {
    ...
    #[test]
    fn test_dwordhhw_conversion() {
        // Test: 0x123456789ABCDEF0
        // DWordHHW: [0x9ABCDEF0, 0x5678, 0x1234]
        let (lo, hi) = dwordhhw_to_dwordwl(0x9ABC_DEF0, 0x5678, 0x1234);
        assert_eq!(lo, 0x9ABC_DEF0);
        assert_eq!(hi, 0x1234_5678);
    }

    #[test]
    fn test_dwordhl_conversion() {
        // Test: 0x123456789ABCDEF0
        // DWordHL: [0xDEF0, 0x9ABC, 0x5678, 0x1234]
        let (lo, hi) = dwordhl_to_dwordwl(0xDEF0, 0x9ABC, 0x5678, 0x1234);
        assert_eq!(lo, 0x9ABC_DEF0);
        assert_eq!(hi, 0x1234_5678);
    }
