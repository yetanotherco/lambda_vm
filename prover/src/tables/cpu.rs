...
#[cfg(test)]
mod tests {
    use super::CpuOperation;
    use executor::vm::instruction::decoding::Instruction;
    use executor::vm::instruction::execution::{
        Accelerator, ECSM_SYSCALL_NUMBER, KECCAK_SYSCALL_NUMBER, SyscallNumbers,
    };
    use executor::vm::logs::Log;

    /// The prover's `ecall_keccak`/`ecall_ecsm` classification in `from_log` must
    /// stay identical to the executor's canonical `SyscallNumbers::accelerator()`.
    /// Constructs the `from_log` inputs a real ECALL produces (a `Log` whose
    /// `src1_val` is a7 = the syscall number, decoded as `Instruction::EcallEbreak`
    /// so `f.ecall` is set) and checks both bools against the canonical classifier.
    /// If the prover ever diverges from `accelerator()`, this fails.
    #[test]
    fn from_log_accelerator_flags_match_canonical_classifier() {
        for n in [KECCAK_SYSCALL_NUMBER, ECSM_SYSCALL_NUMBER] {
            let log = Log {
                current_pc: 0,
                next_pc: 4,
                src1_val: n,
            ...
            let op = CpuOperation::from_log_and_instruction(&log, 0, Instruction::EcallEbreak);
            let accel = SyscallNumbers::try_from(n).unwrap().accelerator();
            assert_eq!(
                op.ecall_keccak,
                matches!(accel, Some(Accelerator::Keccak)),
                "ecall_keccak mismatch for syscall {n:#x}"
            );
            assert_eq!(
                op.ecall_ecsm,
                matches!(accel, Some(Accelerator::Ecsm)),
                "ecall_ecsm mismatch for syscall {n:#x}"
            );
        }
    }
}
