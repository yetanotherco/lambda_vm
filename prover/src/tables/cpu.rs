...
pub fn bus_interactions() -> Vec<BusInteraction> {
    ...
    // =========================================================================
    // TEMPORARILY DISABLED: These require the MEMW/LOAD tables to have
    // matching receivers with appropriate multiplicities. Also requires
    // memory state tracking in trace_builder.rs.
    //
    // M1: Register read rs1 via MEMW (multiplicity = read_register1)
    // M3: Register read rs2 via MEMW (multiplicity = read_register2)
    // M5: Register write rd via MEMW (multiplicity = write_register)
    // M6: LOAD operation via LOAD table (multiplicity = LOAD)
    // M7: STORE operation via MEMW (multiplicity = STORE)
    #[allow(clippy::never_loop)]
    for _ in 0..0 {
        // ---------------------------------------------------------------------
        // M1: MEMW[rv1; 1, 2*rs1, rv1, timestamp+0, 0, 0, 1] | read_register1
        // ---------------------------------------------------------------------
        ...
        interactions.push(BusInteraction::sender(
            ...
            vec![
                ...
                BusValue::constant(0),
                // write2=0, write4=0, write8=1 (register = 8 bytes)
                BusValue::constant(0),
                BusValue::constant(0),
                BusValue::constant(1),
            ],
        ...
        // ---------------------------------------------------------------------
        // Write to rd register via MEMW (no old value in bus, MEMW fills it)
        interactions.push(BusInteraction::sender(
            BusId::Memw,
            ...
            vec![
                ...
                // base_address[1] = 0
                BusValue::constant(0),
                // value[0] = rvd[0], value[1] = rvd[1], value[2..7] = 0
        ...
        // ---------------------------------------------------------------------
        // Load operation sends to LOAD table
        // base_address = rv1 + imm (computed as res for LOAD ops)
        interactions.push(BusInteraction::sender(
            BusId::Load,
            Multiplicity::Column(cols::LOAD),
            vec![
                // base_address = res (computed as rv1 + imm in trace)
                // For now using res as DWordBL
                BusValue::Packed { start_column: cols::RES[0], packing: Packing::DWordBL },
                // timestamp
                BusValue::Packed { start_column: cols::TIMESTAMP, packing: Packing::Direct },
                BusValue::constant(0), // timestamp[1] = 0
                // read flags (memory_2bytes, memory_4bytes, memory_8bytes)
                BusValue::Packed { start_column: cols::MEMORY_2BYTES, packing: Packing::Direct },
                BusValue::Packed { start_column: cols::MEMORY_4BYTES, packing: Packing::Direct },
                BusValue::Packed { start_column: cols::MEMORY_8BYTES, packing: Packing::Direct },
                // signed flag
                BusValue::Packed { start_column: cols::SIGNED, packing: Packing::Direct },
                // result (rvd as the loaded value)
                BusValue::Packed { start_column: cols::RVD_0, packing: Packing::Direct },
                BusValue::Packed { start_column: cols::RVD_1, packing: Packing::Direct },
            ],
        ));
        ...
        // ---------------------------------------------------------------------
        // M7: MEMW[0, base_address, rv2, timestamp+1, mem_2, mem_4, mem_8] | STORE
        // ---------------------------------------------------------------------
        // Store operation sends to MEMW (memory write)
        interactions.push(BusInteraction::sender(
            BusId::Memw,
            ...
            vec![
                ...
                BusValue::constant(0),
                // base_address = res (computed as rv1 + imm)
                BusValue::Packed { start_column: cols::RES[0], packing: Packing::DWordBL },
                // value = rv2 (packed as WL: lo32, hi32, 0, 0, 0, 0, 0, 0)
                BusValue::linear(vec![
                    LinearTerm::Column { coefficient: 1, column: cols::RV2_0 },
                    LinearTerm::Column { coefficient: 65536, column: cols::RV2_1 },
                ]),
