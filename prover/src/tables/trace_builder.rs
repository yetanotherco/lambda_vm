...
use super::fp3_mul::{self, Fp3MulOperation};
use super::halt;
...
///
/// Returns: (memw_ops, load_ops, lt_ops, shift_ops, bitwise_ops, commit_ops, keccak_ops, fp3_mul_ops)
...
fn collect_ops_from_cpu(
    ...
    for op in cpu_ops {
        ...
        // Collect Fp3Mul ECALL operations
        if op.ecall_fp3_mul {
            let fp3_op = collect_fp3_mul_memw_ops(op, memory_state, register_state, &mut memw_ops);
            fp3_mul_ops.push(fp3_op);
        }
...
pub struct Traces {
    ...
    /// FP3_MUL table (one row per Fp3Mul precompile call)
    pub fp3_mul: TraceTable<GoldilocksField, GoldilocksExtension>,
...
struct CollectedOps {
    ...
    fp3_mul_ops: Vec<Fp3MulOperation>,
}
...
fn build_traces(
    ...
    // Generate the FP3_MUL trace (one row per Fp3Mul precompile call).
    let fp3_mul_trace = fp3_mul::generate_fp3_mul_trace(&fp3_mul_ops);
...
impl Traces {
    ...
    pub fn total_field_elements(&self) -> u64 {
        ...
        total += (fp3_mul.num_rows() * FP3_MUL_COLS) as u64;
        total
    }
    ...
    pub fn total_auxiliary_field_elements(&self) -> u64 {
        ...
        let n_fp3_mul = aux_cols(super::fp3_mul::bus_interactions().len());
        ...
        total += (fp3_mul.num_rows() * n_fp3_mul) as u64;
        total
    }
    ...
    pub fn from_elf_and_logs(
        ...
        memory_state.add_private_input(private_input);
        let mut register_state = RegisterState::new(elf.entry_point);
        let (memw_ops, load_ops, lt_ops, shift_ops, bitwise_ops, commit_ops, keccak_ops, fp3_mul_ops) =
...
mod keccak_tests {
    ...
    fn test_keccak_round_trace_matches_f1600() {
        ...
        for round in 0..24 {
            let rc = executor::constants::KECCAK_RC[round];
            let mut c = [0u64; 5];
            for x in 0..5 {
                c[x] = ref_state[x]
                    ^ ref_state[x + 5]
            ...
            for x in 0..5 {
                for y in 0..5 {
                    b[y + 5 * ((2 * x + 3 * y) % 5)] =
                        ref_state[x + 5 * y].rotate_left(executor::constants::KECCAK_RHO[x][y]);
                }
            }
            for x in 0..5 {
                for y in 0..5 {
