...
impl WalkLeftover {
    ...
    pub(crate) fn build_accumulated(&self) -> AccumulatedTables {
        ...
        // The same rule `generate_optional` applies on the all-at-once path: no
        // ops, no table. The two have to agree — the walk and the ordinary build
        // are compared chunk for chunk.
        fn accel_of<T>(
            ops: &[T],
...
impl CollectedOps {
    ...
    pub(crate) fn build_table(
        ...
        macro_rules! build {
            ($ops:expr, $limit:expr, $f:path) => {
                // One predicate, read here and by `num_chunks`: a kind that is
                // elided when empty must be counted as elided, or the placeholder
                // count and the real build disagree about the layout.
                if kind.always_present() {
                    chunk_and_generate(
                        $ops,
                        $limit,
                        $f,
    ...
    ///
    /// Mirrors `build_table` through the same `always_present` predicate: an
    /// empty op list yields one padded chunk for the kinds that are always in
    /// the proof and none at all for the kinds that are elided when empty.
    pub(crate) fn num_chunks(&self, kind: TableKind, max_rows: &super::MaxRowsConfig) -> usize {
        let (len, limit) = self.shape_of(kind, max_rows);
        if len == 0 {
            usize::from(kind.always_present())
        } else {
            len.div_ceil(limit)
        }
    }
...
impl TableKind {
    /// Whether this table is in the proof even when its op list is empty.
    ///
    /// CPU and MEMW_R are: every run executes instructions and every run reads
    /// registers, so their padded single chunk is never wasted. Every other
    /// chunked kind is left out entirely when it has no ops — a chip reaches the
    /// run only through its LogUp bus, and a chip with no rows sends nothing.
    /// See `chunk_and_generate_optional` for why that is sound.
    ///
    /// The one predicate both `build_table` and `num_chunks` read. They have to
    /// agree: `num_chunks` sizes the placeholder layout a retired trace is
    /// proved against, and `build_table` produces the tables that fill it.
    pub(crate) const fn always_present(self) -> bool {
        matches!(self, TableKind::Cpu | TableKind::MemwRegister)
    }
}
