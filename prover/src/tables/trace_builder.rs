...
impl Traces {
    ...
    pub fn from_elf_and_logs(
        ...
        // disk-spill: collect register and aligned ops from SpilledVec; rest stays for chunk processing.
        #[cfg(feature = "disk-spill")]
        let memw_register_ops: Vec<MemwOperation> = {
            let mut register_ops: Vec<MemwOperation> = memw_ops
                .as_slice()
                .iter()
                .filter(|op| is_register_op(op))
                .cloned()
                .collect();
            register_ops.extend(
                halt_memw_ops
        ...
        let memw_aligned_ops: SpilledVec<MemwOperation> = {
            ...
            for op in memw_ops
                .as_slice()
                .iter()
                .filter(|op| !is_register_op(op) && is_aligned_op(op))
            {
                writer
                    .push(op.clone())
                    .map_err(|e| Error::Prover(format!("disk-spill memw_aligned: {e}")))?;
            }
            for op in halt_memw_ops
                .iter()
                .filter(|op| !is_register_op(op) && is_aligned_op(op))
            {
                writer
        ...
        #[cfg(feature = "disk-spill")]
        {
            ...
            bitwise::update_multiplicities(
                &mut bitwise_table,
                &collect_bitwise_from_memw_register(&memw_register_ops),
            );
            bitwise::update_multiplicities(
                &mut bitwise_table,
    ...
    pub fn from_logs(
        ...
        // HALT finalization: 33 register MEMW operations at timestamp u64::MAX.
        let halt_memw_ops = collect_halt_ops(&mut register_state);
        // Unify memw_ops for both disk-spill (SpilledVec) and non-disk-spill (Vec).
        #[cfg(feature = "disk-spill")]
        let memw_combined = {
        ...
        // Route MEMW_R (register fast-path) first, then MEMW_A (aligned), rest → MEMW.
        let (memw_register_ops, memw_combined): (Vec<_>, Vec<_>) =
            memw_combined.into_iter().partition(is_register_op);
        let (memw_aligned_ops, memw_ops): (Vec<MemwOperation>, Vec<MemwOperation>) =
