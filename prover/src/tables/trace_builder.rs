...
fn collect_bitwise_from_keccak(keccak_ops: &[KeccakOperation]) -> Vec<BitwiseOperation> {
    ...
    for kop in keccak_ops {
        ...
        for round in 0..24 {
            ...
            // iota: XOR_BYTE (4)
            // Only the 4 non-zero rc bytes are stored and XORed; the other
            // 4 bytes have rc = 0 so iota equals chi[0][0][b] directly.
            let rc_val = KECCAK_RC[round];
            for &b in &super::keccak_rnd::cols::RC_NONZERO_BYTES {
...
fn build_traces(
    ...
    let branches = chunk_and_generate(&branch_ops, max_rows.branch, branch::generate_branch_trace);
...
mod keccak_tests {
    ...
    fn test_keccak_bitwise_ops_count() {
        ...
        // Drop rc[2,4,5,6]: 4 fewer XOR_BYTE per round (iota stage).
        ...
        for round in 0..24 {
            ...
            for (lane, &lane_val) in ref_state.iter().enumerate() {
                ...
                for byte_idx in 0..8 {
                    ...
                    let col = rnd_cols::output_byte_col(x, y, byte_idx);
        ...
        for x in 0..5 {
            for y in 0..5 {
                for b in 0..8 {
                    ...
                    let rnd_val = &rnd_trace.main_table.data
                        [rnd_base_23 + rnd_cols::output_byte_col(x, y, b)];
