...
/// BITWISE lookups emitted by the KECCAK **core** chip: the state-address byte
/// checks and the 100 IS_HALF state-pointer halfword checks per call.
///
/// Split from the round chip's lookups because the two chips can live in
/// different proofs: a continuation epoch keeps the core (it does the memory
/// I/O) but hoists the round chip into the global proof, so each half's lookups
/// must follow its own chip to the right BITWISE table.
...
/// replaying the 24 rounds. These follow the round chip: in a continuation they
/// are collected into the GLOBAL proof's BITWISE table, not the epoch's.
...
impl CollectedEpoch {
    ...
    /// The epoch's keccak permutation calls, for the global proof: it rebuilds
    /// this epoch's KECCAK core table (whose committed root must equal the one
    /// the epoch proof publishes) and folds the ops into the run-wide KECCAK_RND
    /// chain. The epoch's own trace build derives its core from the very same
...
fn build_traces<I: ImageSource + Sync>(
    ...
    // Continuation epochs hoist the keccak round chip out: the epoch commits its
    // KECCAK core without the `Keccak` bus pair, and one run-wide KECCAK_RND in
    // the global proof answers every epoch's requests. KECCAK_RND is a pure
    // function — no memory, no ordering, no state between calls — so one
    // instance suffices. This is exactly "is a continuation epoch", the same
    // condition as the L2G bookend.
    ...
    let gen_keccak_rnds = || {
        // Hoisted: the round chip is proved once in the global proof, so this
        // epoch emits no KECCAK_RND table at all. `table_counts` is derived from
        // these vectors, so the count follows automatically.
        if hoist_keccak_rounds {
            return Ok(Vec::new());
        }
