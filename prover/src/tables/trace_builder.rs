...
use super::halt;
use super::hint;
use super::is_b48::{self, B48Operation};
...
/// The `IS_B48` requests the keccak core chip sends: the top 48 bits of every
/// lane pointer.
///
/// All 25 lanes of one call share a value unless the 200-byte state straddles a
/// 64 KiB boundary, in which case the run produces two — which is exactly the
/// repetition the IS_B48 chip exists to collapse.
pub(crate) fn collect_b48_from_keccak(keccak_ops: &[KeccakOperation]) -> Vec<B48Operation> {
    let mut ops = Vec::with_capacity(keccak_ops.len() * 25);
    for kop in keccak_ops {
        for lane_idx in 0..25u64 {
            let ptr = kop
                .state_addr
                .checked_add(lane_idx * 8)
                .expect("keccak state address range must be validated by the executor");
            ops.push(B48Operation::from_address(ptr));
        }
    }
    ops
}

/// Count the BITWISE lookups sent by the IS_B48 chip itself.
///
/// The chip sends `IS_HALF` on each of its three limbs with multiplicity `μ`,
/// not 1, so this bumps by `μ` rather than pushing one op per request. It reads
/// the same merged `(value, μ)` pairs as [`is_b48::generate_is_b48_trace`], so
/// the multiplicities it reports and the ones the trace carries cannot drift.
pub(crate) fn collect_bitwise_from_b48(
    b48_ops: &[B48Operation],
    hist: &mut bitwise::BitwiseHistogram,
) {
    for (op, multiplicity) in is_b48::merge_operations(b48_ops) {
        for limb in op.limbs() {
            hist.bump_n(is_half_op(limb), multiplicity);
        }
    }
}
...
pub(crate) fn collect_bitwise_from_keccak(keccak_ops: &[KeccakOperation]) -> Vec<BitwiseOperation> {
    ...
    for kop in keccak_ops {
        ...
        // IS_HALF for the low limb of each state_ptr (25 per call). The top 48
        // bits go to the IS_B48 chip instead — see `collect_b48_from_keccak`,
        // whose own IS_HALF sends are counted by `collect_bitwise_from_b48`.
        for lane_idx in 0..25 {
            let ptr = state_addr
                .checked_add(lane_idx as u64 * 8)
                .expect("keccak state address range must be validated by the executor");
            let half = (ptr & 0xFFFF) as u16;
            ops.push(BitwiseOperation::halfword(
                BitwiseOperationType::IsHalf,
                (half & 0xFF) as u8,
                ((half >> 8) & 0xFF) as u8,
...
pub struct Traces {
    ...
    /// IS_B48 range-check table (one row per distinct 48-bit value asked about).
    /// Empty when no chip sends on the `IsB48` bus — today that means: when the
    /// run makes no keccak call.
    pub is_b48s: Vec<TraceTable<GoldilocksField, GoldilocksExtension>>,
...
fn build_traces<I: ImageSource + Sync>(
    ...
    // IS_B48 requests are a pure function of the keccak ops (the top 48 bits of
    // each lane pointer), so they are derived here rather than carried through
    // `CollectedOps` — the same way the keccak round ops are. Phase 4 needs them
    // for the BITWISE multiplicities and phase 5 builds the table from them.
    let b48_ops = collect_b48_from_keccak(&keccak_ops);
    ...
    let mut collectors: Vec<Collector> = vec![
        ...
        Box::new(|h| collect_bitwise_from_b48(&b48_ops, h)),
    ...
    // IS_B48 range-check table. Absent entirely when nothing sends on the bus.
    let gen_is_b48s = || {
        generate_optional(
            &b48_ops,
            is_b48::generate_is_b48_trace,
            #[cfg(feature = "disk-spill")]
            storage_mode,
        )
    };
    ...
    let (mut hints_slot, mut is_b48s_slot) = (None, None);
    ...
    if !sequential {
        ...
        rayon::scope(|s| {
            ...
            spawn_into!(is_b48s_slot, gen_is_b48s);
        });
    } else {
    ...
    let is_b48s = is_b48s_slot.expect(PHASE5_RAN)?;
...
impl Traces {
    ...
    pub fn total_field_elements(&self) -> u64 {
        ...
        use super::is_b48::cols::NUM_COLUMNS as IS_B48_COLS;
        ...
        for t in is_b48s {
            total += (t.num_rows() * IS_B48_COLS) as u64;
        }
        total
    }
    ...
    pub fn total_auxiliary_field_elements(&self) -> u64 {
        ...
        let n_is_b48 = aux_cols(super::is_b48::bus_interactions().len());
        ...
        for t in is_b48s {
            total += (t.num_rows() * n_is_b48) as u64;
        }
        total
    }
