...
pub fn memw_domain_eval<CB: ConstraintBuilder>(cb: &mut CB) {
    ...
    // idx 4..=9: IS_BIT<carry[i]>
    for &col in &cols::CARRY {
        let v = cb.main(col).clone();
        cb.fold(&v * (&one - &v));
    }
    // IS_BIT<write2/write4/write8>
    for &col in &[cols::WRITE2, cols::WRITE4, cols::WRITE8] {
        let v = cb.main(col).clone();
        cb.fold(&v * (&one - &v));
    }
    // WidthSumIsBit — w2 * (1 - w2)
    let w2 = cb.main(cols::WRITE2) + cb.main(cols::WRITE4) + cb.main(cols::WRITE8);
    cb.fold(&w2 * (&one - &w2));
}

/// MEMW's migrated domain constraints as an object-safe `TableConstraints`.
pub struct MemwDomain;

impl TableConstraints<GoldilocksField, GoldilocksExtension> for MemwDomain {
    fn eval_prover(
        &self,
