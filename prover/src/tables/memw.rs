...
pub fn generate_memw_trace(
    ...
    // GPU fast path: pack each op into seven parallel arrays
    // (value/old/old_ts 8-stride interleaved); kernel computes carry +
    // row layout.
    #[cfg(feature = "cuda")]
    if let Some(table) = try_generate_memw_trace_gpu(operations, num_rows) {
        return table;
    }
...
/// CUDA fast path for `generate_memw_trace`. Packs seven parallel host
/// buffers; `value`/`old`/`old_timestamp` use 8-stride interleaving.
#[cfg(feature = "cuda")]
fn try_generate_memw_trace_gpu(
    operations: &[MemwOperation],
    num_rows: usize,
    ...
    for (i, op) in operations.iter().enumerate() {
        ...
        for j in 0..8 {
            ...
            olds[off + j] = op.old[j];
            old_timestamps[off + j] = op.old_timestamp[j];
        }
        let (w2, w4, w8) = op.write_flags();
        let mut f: u64 = 0;
        if op.is_register {
    ...
    let raw = stark::gpu_lde::try_generate_memw_trace_gpu_raw(
        num_rows,
        &base_addresses,
        &timestamps,
        &values,
