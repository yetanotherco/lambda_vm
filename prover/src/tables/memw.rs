...
pub fn bus_interactions() -> Vec<BusInteraction> {
    ...
    // IS_HALFWORD range checks for address_add[i][j]
    // -------------------------------------------------------------------------
    // Each address_add[i] is 4 halfwords (DWordHL), need to range check all of them.
    ...
    for _ in 0..0 {
        ...
        // NOTE: Multiplicity should be w2 = write2 + write4 + write8, but Multiplicity::Linear
        // is not available. Using Sum as approximation (code is disabled anyway).
        interactions.push(BusInteraction::sender(
            BusId::Memory,
