...
pub(crate) fn static_zero_page_commitment(blowup_factor: u8) -> Option<Commitment> {
    match blowup_factor {
        2 => Some([
            0x3b, 0x8d, 0xfb, 0xfc, 0xab, 0xcb, 0x13, 0x98, 0x1b, 0xec, 0xe7, 0x72, 0x38, 0x50,
            0xcc, 0xd1, 0x9a, 0x95, 0xe6, 0xcc, 0x6c, 0xf7, 0xf0, 0x4a, 0x49, 0xc9, 0x5d, 0x52,
            0x42, 0x2b, 0x8c, 0x4b,
        ]),
        4 => Some([
            0x0b, 0x95, 0xb7, 0xe1, 0x62, 0xe4, 0xec, 0x59, 0xd5, 0xb7, 0x41, 0x7f, 0xc6, 0xe7,
            0x03, 0x7c, 0xee, 0x3c, 0xc2, 0x10, 0x29, 0xea, 0xbf, 0xae, 0x5a, 0xfd, 0x5e, 0x5d,
            0x00, 0x0b, 0x41, 0xdb,
        ]),
        8 => Some([
            0x20, 0x86, 0x24, 0x23, 0x30, 0x0a, 0xea, 0xdc, 0x58, 0x37, 0x88, 0xf7, 0x33, 0x39,
            0x89, 0x2c, 0xf5, 0x2b, 0xfa, 0x89, 0x8b, 0x06, 0x60, 0x4d, 0x6f, 0x21, 0xc7, 0xfb,
            0x6f, 0xcb, 0xac, 0x84,
        ]),
        _ => None,
    }
...
/// The ONE-ROW (S2) twin of [`static_zero_page_commitment`]: the same columns committed with one
...
/// test. The same regeneration rules as [`static_zero_page_commitment`]. A blowup with no arm here
/// is a hard miss under one row (RULINGS 14): no recompute.
pub(crate) fn static_zero_page_commitment_one_row(blowup_factor: u8) -> Option<Commitment> {
    match blowup_factor {
        4 => Some([
            0x19, 0x19, 0x77, 0x25, 0x76, 0x36, 0xfc, 0x2e, 0xa9, 0xaf, 0xb5, 0x0a, 0x11, 0x93,
            0xe7, 0x8f, 0xe2, 0x58, 0x38, 0x7a, 0x36, 0x4d, 0xf5, 0xad, 0x72, 0x91, 0x2d, 0x43,
            0xee, 0xca, 0xfa, 0xe4,
        ]),
        _ => None,
    }
...
pub(crate) fn static_private_page_commitment(blowup_factor: u8) -> Option<Commitment> {
    match blowup_factor {
        2 => Some([
            0x04, 0x31, 0x10, 0xb3, 0x61, 0xbd, 0xd4, 0x6d, 0x92, 0x65, 0x0e, 0x2d, 0xc9, 0xe6,
            0x1d, 0x20, 0x78, 0xc3, 0xd6, 0xc6, 0x8f, 0x12, 0xb0, 0x45, 0xf0, 0x76, 0x68, 0x27,
            0x5e, 0x2b, 0xa7, 0xbd,
        ]),
        4 => Some([
            0xef, 0x71, 0xa3, 0x11, 0x4a, 0xb3, 0xf7, 0xd6, 0x77, 0xe4, 0x79, 0x0e, 0xd1, 0x96,
            0xf3, 0x36, 0x19, 0x70, 0x6b, 0xd7, 0x2e, 0x04, 0x86, 0x13, 0xfd, 0x81, 0xd1, 0xf4,
            0x26, 0xca, 0xd2, 0x41,
        ]),
        8 => Some([
            0xf7, 0x2d, 0x07, 0xc5, 0x84, 0x5c, 0xc8, 0x92, 0x79, 0x75, 0xf3, 0x56, 0xb7, 0xf9,
            0xe5, 0xd5, 0xb5, 0x8f, 0x62, 0x63, 0x36, 0x8f, 0xd0, 0x8c, 0x42, 0xce, 0xfa, 0x2c,
            0xd2, 0x79, 0xd2, 0x8e,
        ]),
        _ => None,
    }
...
/// The ONE-ROW (S2) twin of [`static_private_page_commitment`]: the same columns committed with one
/// LDE row per leaf ([`LeafLayout::Row`]), per `blowup_factor` in
/// [`crate::tables::STATIC_BLOWUP_FACTORS_ONE_ROW`], generated by
/// `compute_static_commitments --layout row` and pinned by the one-row drift
/// test. The same regeneration rules as [`static_private_page_commitment`]. A blowup with no arm here
/// is a hard miss under one row (RULINGS 14): no recompute.
pub(crate) fn static_private_page_commitment_one_row(blowup_factor: u8) -> Option<Commitment> {
    match blowup_factor {
        4 => Some([
            0x59, 0x6a, 0x3c, 0xc9, 0x79, 0x61, 0x9d, 0x33, 0xa2, 0xcc, 0xfc, 0xba, 0xc3, 0xd9,
            0x6f, 0x84, 0xb0, 0x4a, 0x48, 0x88, 0x87, 0x5d, 0x37, 0x18, 0xa3, 0xfb, 0xd7, 0xce,
            0x2f, 0x5d, 0x96, 0x39,
        ]),
        _ => None,
    }
...
pub fn compute_precomputed_commitment(config: &PageConfig, options: &ProofOptions) -> Commitment {
    compute_precomputed_commitment_with(config, options, LeafLayout::RowPair)
}

/// [`compute_precomputed_commitment`] under an explicit trace-tree leaf layout
/// (S2). ELF data pages have no static root, so a one-row data page computes
/// this at run time.
...
pub fn compute_offset_only_commitment(options: &ProofOptions) -> Commitment {
    compute_offset_only_commitment_with(options, LeafLayout::RowPair)
}

/// [`compute_offset_only_commitment`] under an explicit leaf layout (S2).
pub fn compute_offset_only_commitment_with(
    options: &ProofOptions,
    layout: LeafLayout,
) -> Commitment {
    ...
    commit_preprocessed_columns(&[offset_col], num_rows, options, layout)
}
...
/// The zero-init PAGE commitment under the table's resolved leaf `layout`:
/// today's [`zero_init_preprocessed_commitment`] for row pairs; for one row the
/// static twin at coset 3, and `None` otherwise (a hard miss, RULINGS 14).
pub fn zero_init_preprocessed_commitment_for(
    options: &ProofOptions,
    layout: LeafLayout,
) -> Option<Commitment> {
    match layout {
...
/// The private-input PAGE commitment under the table's resolved leaf `layout`
/// (see [`zero_init_preprocessed_commitment_for`]).
pub fn private_page_preprocessed_commitment_for(
    options: &ProofOptions,
    layout: LeafLayout,
) -> Option<Commitment> {
    match layout {
...
/// The zero-init page's commitment source for both leaf layouts.
pub fn zero_init_lazy_commitment(options: &ProofOptions) -> stark::lookup::LazyCommitment {
    zero_init_lazy_commitment_from(zero_init_preprocessed_commitment(options), options)
}

/// [`zero_init_lazy_commitment`] with the row-pair root already in hand.
pub fn zero_init_lazy_commitment_from(
    row_pair: Commitment,
    options: &ProofOptions,
) -> stark::lookup::LazyCommitment {
    let o = options.clone();
    stark::lookup::LazyCommitment::ready(row_pair)
        .with_one_row(move || zero_init_preprocessed_commitment_for(&o, LeafLayout::Row))
}

/// The private-input page's (OFFSET-only) commitment source for both layouts.
pub fn private_page_lazy_commitment(options: &ProofOptions) -> stark::lookup::LazyCommitment {
    let o = options.clone();
    stark::lookup::LazyCommitment::ready(private_page_preprocessed_commitment(options))
        .with_one_row(move || private_page_preprocessed_commitment_for(&o, LeafLayout::Row))
}

/// An ELF data page's commitment source: the row-pair root `supplied` by the
/// caller or computed on first use, and the one-row root computed on first use
/// (program-dependent, so there is no static twin; a supplied root is a
/// row-pair root and never stands in for the other layout).
pub fn data_page_lazy_commitment(
    config: &PageConfig,
    options: &ProofOptions,
