...
//!
//! 24 rows (one per round), padded to 32. Each row maps a round index to the
//! 4 non-zero bytes of its round constant. The round chip looks up
//! `(round) → rc[0], rc[1], rc[3], rc[7]` via the `KeccakRc` bus.
//!
//! The 24 Keccak round constants are derived from an LFSR and only ever have
//! bits set at positions {0, 1, 3, 7, 15, 31, 63}, so bytes {2, 4, 5, 6} of
//! every constant are zero and need not be stored or XORed (spec keccak.typ
...
pub mod cols {
    ...
    /// RC bytes [0..3] — the 4 non-zero bytes of the round constant.
    /// Index `i` maps to actual byte position `RC_NONZERO_BYTES[i]`.
    pub const RC: usize = 1;
    pub const RC_END: usize = RC + 4; // = 5
    ...
    /// rounds. The other byte positions {2, 4, 5, 6} are constant zero and
    /// not stored.
...
/// Generate one precomputed row: [round, rc[byte0], rc[byte1], rc[byte3], rc[byte7]].
/// Only the non-zero rc bytes are stored (see `cols::RC_NONZERO_BYTES`).
...
///
/// Format: [round, rc[0], rc[1], rc[3], rc[7]] (all `Direct`).
/// Bytes {2, 4, 5, 6} of every round constant are zero and not stored.
