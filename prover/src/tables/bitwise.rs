...
/// The ONE-ROW (S2) twin of [`static_commitment`]: the same columns committed
/// with one LDE row per leaf ([`LeafLayout::Row`]), per `blowup_factor` in
...
/// test. The same regeneration rules as [`static_commitment`]. A blowup with
/// no arm here is a hard miss under one row (RULINGS 14): no recompute.
pub(crate) fn static_commitment_one_row(blowup_factor: u8) -> Option<Commitment> {
    match blowup_factor {
        4 => Some([
            0x34, 0x22, 0x21, 0x59, 0xc3, 0xe7, 0x92, 0x18, 0xb5, 0xf0, 0x3b, 0xd8, 0x73, 0x37,
            0xd8, 0x33, 0x62, 0xbb, 0xea, 0xe4, 0x1f, 0x1e, 0x0a, 0x15, 0x59, 0x19, 0x8c, 0xf2,
            0x13, 0xd4, 0x82, 0x09,
        ]),
        _ => None,
    }
...
/// hard miss the prover refuses and the verifier rejects (RULINGS 14), never a
/// recompute of a 2^20-row table behind the operator's back.
