...
pub fn bus_interactions() -> Vec<BusInteraction> {
    ...
    // -------------------------------------------------------------------------
    // Signature (24 elements):
    // [old[0], old[1], 0,0,0,0,0,0, 1(is_reg), 2*ADDRESS(addr_lo), 0(addr_hi),
    //  val[0], val[1], 0,0,0,0,0,0, timestamp_0, timestamp_1,
    //  1(write2), 0(write4), 0(write8)]
    ...
    // -------------------------------------------------------------------------
    // Signature (16 elements):
    // [1(is_reg), 2*ADDRESS(addr_lo), 0(addr_hi),
    //  val[0], val[1], 0,0,0,0,0,0,
    //  timestamp_0, timestamp_1, 1(write2), 0(write4), 0(write8)]
    interactions.push(BusInteraction::receiver(
        BusId::Memw,
...
pub fn constraints() -> Vec<Box<dyn TransitionConstraint<GoldilocksField, GoldilocksExtension>>> {
    ...
    vec![
        Box::new(IsBitConstraint::unconditional(cols::MU_READ, 0)),
        Box::new(IsBitConstraint::unconditional(cols::MU_WRITE, 1)),
        Box::new(MemwRegisterMuSumIsBit::new(2)),
    ]
}
