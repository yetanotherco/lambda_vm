...
///   enforced by `is_register_op`). With single-`Word` timestamps this low limb is the
///   full old timestamp (register old_timestamps are always < 2^32), stored in the
///   single `OLD_TIMESTAMP` column.
...
pub(crate) fn generate_memw_register_trace_from_rows(
    ...
    for (row_idx, r) in rows.iter().enumerate() {
        ...
        table.set_u64(row_idx, cols::TIMESTAMP, r.timestamp);
        ...
        // Old timestamp: single Word (registers' old_timestamp < 2^32).
        table.set_u64(row_idx, cols::OLD_TIMESTAMP, r.old_ts_lo as u64);
        // Multiplicity.
