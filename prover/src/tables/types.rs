...
pub enum BusId {
    ...
    KeccakRc,
    /// Fp3Mul precompile ECALL receiver (shares the ECALL bus is not used here;
    /// this id is reserved for any future dedicated Fp3Mul bus). The current
    /// Fp3Mul wiring uses the shared `Ecall` and `Memw` buses directly.
    Fp3Mul,
}
