...
/// V5 appends `TableCounts::keccak_rnd`, which made KECCAK_RND a chunked table
/// like CPU or MEMW instead of a single instance — so the count is now part of
/// the AIR set the verifier rebuilds, exactly as the other chunk counts are.
...
pub(crate) const CONTINUATION_EPOCH_TAG: &[u8] = b"LAMBDAVM_CONTINUATION_EPOCH_V4";
