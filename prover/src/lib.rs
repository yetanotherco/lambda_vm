...
/// metadata fields. Both [`verify_with_options`] (owned proof) and
/// [`verify_recursion_blob`] (guest blob, zero-copy) funnel here, passing a
/// [`StarkProofView`] slice over their respective (owned or archived) proof
/// data — no serialization, no duplicated verification logic.
#[allow(clippy::too_many_arguments)]
