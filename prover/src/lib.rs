...
pub(crate) struct VmAirs {
    ...
    /// KECCAK core tables. When [`Self::hoist_keccak_rounds`] is set these are
    /// built WITHOUT the `Keccak` bus pair — nothing in this proof would answer
    /// it, because the round chip lives in the global proof.
    pub keccaks: Vec<VmAir>,
    /// Empty when [`Self::hoist_keccak_rounds`] is set.
    ...
    /// continuation epochs: KECCAK_RND is proved ONCE for the whole run in the
    /// global proof, and each epoch's core simply omits its `Keccak` bus pair.
    /// KECCAK_RND is a pure function — no memory, no ordering, no state between
    /// calls — so one run-wide instance can serve every epoch. The epoch's core
    /// trace is the carrier: the global proof re-emits the bus pair over that
    /// same committed trace, and the two are tied by comparing its main-trace
    /// Merkle root (see `continuation::verify_continuation_view`).
...
impl VmAirs {
    ...
    pub fn new(
        ...
        let commits: Vec<_> = (0..table_counts.commit)
            ...
            .collect();
        // `hoist_keccak_rounds` is set after construction (see
        // `with_hoisted_keccak_rounds`), so the caller rebuilds these; the
        // default set keeps the round request.
    ...
    /// Hoist the keccak round chip out of this proof: KECCAK_RND drops to zero
    /// tables and each KECCAK core stops emitting its `Keccak` bus pair, since
    /// nothing here would answer it. The global proof re-emits that pair over
    /// the same committed core traces, against one run-wide round chip.
    ///
    /// Only valid for a continuation epoch. Prover and verifier must set this
    /// identically — it changes both the AIR set and what each core emits.
    pub fn with_hoisted_keccak_rounds(mut self, proof_options: &ProofOptions) -> Self {
        self.hoist_keccak_rounds = true;
        self.keccaks = (0..self.keccaks.len())
            .map(|i| {
                Box::new(
                    create_keccak_air_without_rounds(proof_options)
                        .with_name(&format!("KECCAK[{i}]")),
                ) as VmAir
            })
            .collect();
        self.keccak_rnds.clear();
        self
    }

    /// Index of the first KECCAK core table in this proof's table list, when the
    /// round chip is hoisted and there is a core to point at. Used to pull the
    /// core's committed root out of an epoch proof so it can be tied to the
    /// global proof's copy of that trace.
    ///
    ...
    /// COMMIT chunks, then the KECCAK cores.
    pub fn keccak_core_index(&self) -> Option<usize> {
        if !self.hoist_keccak_rounds || self.keccaks.is_empty() {
            return None;
        }
        Some(4 + usize::from(self.include_halt) + self.commits.len())
    }
}
...
/// table list: 0 for L2G, `E` for the KECCAK cores (see `prove_global`).
