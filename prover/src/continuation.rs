...
/// sim/27 SIM_CONSTRAINT_EVAL v2 support (host-only, MEASUREMENT-ONLY). Breaks
/// the executor↔stark dependency cycle by capturing each table's constraint
/// program HERE (concrete `F`/`E`, where the interpreter lives) and evaluating it
/// on demand. The CLI drives both halves: it runs the host verify once with
/// capture active to preload the programs in the guest's exact compute_transition
/// order, then registers [`eval`] as the executor's constraint evaluator. NEVER
/// used on the guest (the ecall replaces `compute_transition` there); this module
/// is compiled out on riscv64.
#[cfg(not(target_arch = "riscv64"))]
pub mod sim_constraint {
    use super::{AirRef, E, F};
    use executor::vm::instruction::sim_midlevel::ConstraintEvalRequest;
    use math::field::element::FieldElement;
    use stark::constraint_ir::{ConstraintProgram, eval_program_verifier};
    ...
    type Base = FieldElement<F>;

    thread_local! {
        static PROGRAMS: RefCell<Option<Vec<ConstraintProgram<F, E>>>> =
            const { RefCell::new(None) };
    }

    /// Begin capturing constraint programs (one per compute_transition).
    pub fn begin_capture() {
        PROGRAMS.with(|p| *p.borrow_mut() = Some(Vec::new()));
    }

    /// Whether a capture pass is active (the `verify_epoch` hook checks this).
    pub fn is_capturing() -> bool {
        PROGRAMS.with(|p| p.borrow().is_some())
    }

    /// Record every AIR's captured program, in order. Called by `verify_epoch`
    /// once per epoch over its `refs` (air_refs + the epoch L2G air) — the same
    /// sequence the guest hits `compute_transition` in.
    pub fn record_refs(refs: &[AirRef<'_>]) {
        PROGRAMS.with(|p| {
            if let Some(v) = p.borrow_mut().as_mut() {
                for r in refs {
                    v.push(r.constraint_program().clone());
                }
            }
        });
    }

    /// Take the captured programs (ends the capture pass).
    pub fn take_captured() -> Vec<ConstraintProgram<F, E>> {
        PROGRAMS.with(|p| p.borrow_mut().take().unwrap_or_default())
    }

    #[inline]
    fn to_ext(l: &[u64; 3]) -> Ext {
        Ext::from_raw([
            Base::from_raw(l[0]),
            Base::from_raw(l[1]),
            Base::from_raw(l[2]),
    ...
    #[inline]
    fn ext_limbs(e: &Ext) -> [u64; 3] {
        let v = e.value();
        [*v[0].value(), *v[1].value(), *v[2].value()]
    }

    /// Evaluate one table's OOD constraints for a [`ConstraintEvalRequest`],
    /// returning `(per-constraint evals as raw limbs, program node count)`.
    /// Reconstructs the OOD frame exactly as `StarkTableView::into_frame` does,
    /// then runs the verifier's constraint IR interpreter — documented
    /// bit-for-bit equal to the compiled `compute_transition`.
    pub fn eval(
        program: &ConstraintProgram<F, E>,
        req: &ConstraintEvalRequest,
    ) -> (Vec<[u64; 3]>, u64) {
        // Rebuild the frame steps from the row-major OOD grid (replicating
        // `StarkTableView::into_frame`: main = row[..num_main], aux = the rest).
        let steps: Vec<TableView<E, E>> = (0..req.height)
            .step_by(req.step_size.max(1))
            .map(|initial| {
                let end = initial + req.step_size;
                let mut main: Vec<Vec<Ext>> = Vec::new();
                let mut aux: Vec<Vec<Ext>> = Vec::new();
                for row_idx in initial..end {
                    let base = row_idx * req.width;
                    let row = &req.frame_data[base..base + req.width];
                    main.push(row[..req.num_main].iter().map(to_ext).collect());
                    aux.push(row[req.num_main..].iter().map(to_ext).collect());
                }
                TableView::new(main, aux)
            })
            .collect();
        let frame = Frame::new(steps);

        let rap: Vec<Ext> = req.rap_challenges.iter().map(to_ext).collect();
        let alpha: Vec<Ext> = req.alpha_powers.iter().map(to_ext).collect();
        let offset = to_ext(&req.table_offset);
        let ctx = TransitionEvaluationContext::new_verifier(&frame, &rap, &alpha, &offset);

        let mut evals = vec![Ext::zero(); req.num_constraints];
        eval_program_verifier(program, &ctx, &mut evals);

        let out = evals.iter().map(ext_limbs).collect();
...
fn verify_epoch(
    ...
    // sim/27 SIM_CONSTRAINT_EVAL v2 (host-only): when the CLI is preloading, capture
    // every table's constraint program in the exact sequence the guest hits
    // `compute_transition` — so the guest's global SIM_CONSTRAINT_SEQ keys the right
    // program. BATCHED two-lane order: `batched_verify_epoch` runs the L2G lane
    // (`verify_rounds_2_to_4` -> one `step_2`) BEFORE the VM round 4
    // (`batched_verify_round_4` -> one `step_2` per VM table, in `vm_refs` order),
    // and rounds 1-3 evaluate no transition constraints. So capture the L2G air
    // FIRST, then the VM tables. No-op otherwise.
    #[cfg(not(target_arch = "riscv64"))]
    if sim_constraint::is_capturing() {
        sim_constraint::record_refs(core::slice::from_ref(&l2g_ref));
        sim_constraint::record_refs(&vm_refs);
    }
...
fn verify_global(
    ...
    // sim/27 SIM_CONSTRAINT_EVAL v2 (host-only): the cross-epoch global-memory
    // verify runs `compute_transition` for these refs AFTER all per-epoch refs, so
    // capture them last — matching the guest's global ecall-index continuation.
    #[cfg(not(target_arch = "riscv64"))]
    if sim_constraint::is_capturing() {
        sim_constraint::record_refs(&refs);
    }
