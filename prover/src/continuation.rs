...
fn verify_epoch(
    ...
    // every table's constraint program in `refs` order — the exact sequence the
    // guest hits `compute_transition` in (per epoch, air_refs then the L2G air) —
    // so the guest's global ecall index keys the right program. No-op otherwise.
    #[cfg(not(target_arch = "riscv64"))]
