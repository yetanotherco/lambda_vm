...
use crate::constants::{
    FP3_DOT_SYSCALL_NUMBER, FP3_FMA_SYSCALL_NUMBER, FP3_MUL_SYSCALL_NUMBER,
    FP3_SCALAR_DOT_SYSCALL_NUMBER, FP3_SCALAR_FMA_SYSCALL_NUMBER,
};
...
pub enum SyscallNumbers {
    ...
    Fp3ScalarDot,
    // FP3_DOT_SYSCALL_NUMBER (u64::MAX - 6): acc += dot(fp3_lhs, fp3_rhs).
    Fp3Dot,
}
...
impl TryFrom<u64> for SyscallNumbers {
    ...
    fn try_from(value: u64) -> Result<Self, Self::Error> {
        match value {
            ...
            v if v == FP3_DOT_SYSCALL_NUMBER => Ok(SyscallNumbers::Fp3Dot),
            _ => Err(()),
        }
    }
}
        ...
        Ok(match self {
            ...
            Instruction::EcallEbreak => {
                ...
                match syscall_number {
                    ...
                    SyscallNumbers::Fp3ScalarFma => {
                        // Scalar × Fp3 fused multiply-add: acc += scalar * rhs
                        // x10 = acc_ptr ([u64;3], in/out), x11 = scalar_ptr ([u64;1]), x12 = rhs_ptr ([u64;3])
                        let addr_acc = registers.read(10)?;
                        let addr_scalar = registers.read(11)?;
                        let addr_rhs = registers.read(12)?;
                        let acc = [
                            memory.load_doubleword(addr_acc)?,
                            memory.load_doubleword(addr_acc + 8)?,
                        ...
                        let scalar = memory.load_doubleword(addr_scalar)?;
                        let rhs = [
                            memory.load_doubleword(addr_rhs)?,
                            memory.load_doubleword(addr_rhs + 8)?,
                            memory.load_doubleword(addr_rhs + 16)?,
                        ];
                        // acc += scalar * rhs: 3 Goldilocks multiplications + 3 additions
                        let c0 = goldilocks_add(acc[0], goldilocks_mul(scalar, rhs[0]));
                        let c1 = goldilocks_add(acc[1], goldilocks_mul(scalar, rhs[1]));
                        let c2 = goldilocks_add(acc[2], goldilocks_mul(scalar, rhs[2]));
                        memory.store_doubleword(addr_acc, c0)?;
                        memory.store_doubleword(addr_acc + 8, c1)?;
                    ...
                    SyscallNumbers::Fp3Dot => {
                        // Fp3-Fp3 dot product: acc += lhs[i] * rhs[i] for i in 0..n
                        // x10=acc_ptr ([u64;3] in/out), x11=lhs_ptr ([u64;3n]),
                        // x12=rhs_ptr ([u64;3n]), x13=n (count)
                        let addr_acc = registers.read(10)?;
                        let addr_lhs = registers.read(11)?;
                        let addr_rhs = registers.read(12)?;
                        let count = registers.read(13)? as usize;
                        ...
                        for i in 0..count {
                            let lhs = [
                                memory.load_doubleword(addr_lhs + (i as u64) * 24)?,
                                memory.load_doubleword(addr_lhs + (i as u64) * 24 + 8)?,
                                memory.load_doubleword(addr_lhs + (i as u64) * 24 + 16)?,
                            ];
                            let rhs = [
                                memory.load_doubleword(addr_rhs + (i as u64) * 24)?,
                                memory.load_doubleword(addr_rhs + (i as u64) * 24 + 8)?,
                                memory.load_doubleword(addr_rhs + (i as u64) * 24 + 16)?,
                            ];
                            let c0 = goldilocks_fp3_mul_c0(lhs, rhs);
                            let c1 = goldilocks_fp3_mul_c1(lhs, rhs);
                            let c2 = goldilocks_fp3_mul_c2(lhs, rhs);
                            acc[0] = goldilocks_add(acc[0], c0);
                            acc[1] = goldilocks_add(acc[1], c1);
                            acc[2] = goldilocks_add(acc[2], c2);
                        }
                        memory.store_doubleword(addr_acc, acc[0])?;
                        memory.store_doubleword(addr_acc + 8, acc[1])?;
                        memory.store_doubleword(addr_acc + 16, acc[2])?;
