...
/// Hint operation selector passed to [`compute_hint`]. These are also the
/// request-log `hint_id`s the guest writes with `syscalls::syscalls::request_hint`.
...
/// One past the largest valid hint selector. Kept in parity with the request-log
/// `hint_id` constants in the `syscalls` crate (`HINT_FIELD_INV` / `HINT_SCALAR_INV`
/// / `HINT_FIELD_SQRT`).
pub const HINT_SELECTOR_BOUND: u64 = 3;

/// Whether `hint_id` names a hint [`compute_hint`] can produce.
pub const fn is_valid_hint_selector(hint_id: u64) -> bool {
    matches!(hint_id, HINT_FIELD_INV | HINT_SCALAR_INV | HINT_FIELD_SQRT)
}

// Every selector below the bound is valid, and the bound itself is not. Appending a
// selector without moving the bound (or vice versa) fails to compile here.
const _: () = {
    let mut id = 0;
...
/// k256's own serialization. This is the host-side answer to a request logged by
/// the guest's `syscalls::syscalls::request_hint`: the host runs it over
/// [`crate::vm::memory::Memory::hint_requests`] and appends the results to the
/// hint arena in the private-input region for a second pass.
///
...
/// change the guest's result. The `_` arm for an unknown `hint_id` is defensive
/// only: the arena is untrusted input, and the guest verifies regardless.
...
/// makes the trace unprovable. Used by the ECSM ecall.
