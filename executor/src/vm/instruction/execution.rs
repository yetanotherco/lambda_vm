use crate::vm::{
    ...
    instruction::sim_midlevel::{
        SIM_CONSTRAINT_EVAL_SYSCALL_NUMBER, SIM_DOMAIN_POINTS_SYSCALL_NUMBER,
        SIM_FOLD_CHAIN_SYSCALL_NUMBER, SIM_POLY_EVAL_SYSCALL_NUMBER, SIM_POW_SYSCALL_NUMBER,
        SIM_REGISTER_COMMIT_SYSCALL_NUMBER, SIM_VERIFY_PATH_BATCH_SYSCALL_NUMBER,
        sim_constraint_eval, sim_domain_points, sim_fold_chain, sim_poly_eval, sim_pow,
        sim_register_commit,
    },
...
pub enum SyscallNumbers {
    ...
    FextBaseMul = 112,
    FextInv = 113,
    // MID-LEVEL accelerator measurement stubs (sim/27). Placeholder discriminants;
    // the actual syscall values are the `SIM_*_SYSCALL_NUMBER` consts in
    // `sim_midlevel`. MEASUREMENT-ONLY, not chips: the CLI tallies each separately
    // and a build emitting them is EXECUTE-ONLY (never proven).
    SimPolyEval = 114,
    SimPow = 115,
    SimFoldChain = 116,
    // sim/27 stub 4 op-count PROXY, not a host offload: the constraint-eval
    // ceiling is not offloadable in the executor-host model (see `sim_midlevel`),
    // so this ecall only TALLIES per-table constraint counts; the guest still
    // runs the real `compute_transition`.
    SimConstraintEval = 117,
    // sim/31: batched primary FRI query evaluation points (one call replaces the
    // per-query `pow`). MEASUREMENT-ONLY, execute-only.
    SimDomainPoints = 118,
    // sim/31: REGISTER preprocessed FFT+LDE+Merkle commitment, served by a
    // CLI-registered evaluator. MEASUREMENT-ONLY, execute-only.
    SimRegisterCommit = 119,
    // sim/34: verify EVERY committed FRI layer's Merkle opening for one query in
    // one call (subsumes the per-layer HASH_FELTS + VERIFY_PATH). Placeholder
    // discriminant; the actual value is SIM_VERIFY_PATH_BATCH_SYSCALL_NUMBER.
    // MEASUREMENT-ONLY, execute-only.
    SimVerifyPathBatch = 120,
}
...
impl TryFrom<u64> for SyscallNumbers {
    ...
    fn try_from(value: u64) -> Result<Self, Self::Error> {
        match value {
            ...
            v if v == SIM_POLY_EVAL_SYSCALL_NUMBER => Ok(SyscallNumbers::SimPolyEval),
            v if v == SIM_POW_SYSCALL_NUMBER => Ok(SyscallNumbers::SimPow),
            v if v == SIM_FOLD_CHAIN_SYSCALL_NUMBER => Ok(SyscallNumbers::SimFoldChain),
            v if v == SIM_CONSTRAINT_EVAL_SYSCALL_NUMBER => Ok(SyscallNumbers::SimConstraintEval),
            v if v == SIM_DOMAIN_POINTS_SYSCALL_NUMBER => Ok(SyscallNumbers::SimDomainPoints),
            v if v == SIM_REGISTER_COMMIT_SYSCALL_NUMBER => Ok(SyscallNumbers::SimRegisterCommit),
            v if v == SIM_VERIFY_PATH_BATCH_SYSCALL_NUMBER => {
                Ok(SyscallNumbers::SimVerifyPathBatch)
            }
            _ => Err(()),
        }
    }
...
impl SyscallNumbers {
    ...
    pub fn accelerator(self) -> Option<Accelerator> {
        match self {
            ...
            // The mid-level accelerator stubs (sim/27) are measurement-only, not chips.
    ...
    pub fn sim_hash_ecall(self) -> Option<SimHashEcall> {
        match self {
            ...
            | SyscallNumbers::RegisterRoLayout
            | SyscallNumbers::ReducedOpeningRowInplace
            // The mid-level accelerator stubs (sim/27) are counted by the CLI's
            // own classifiers, not here.
            | SyscallNumbers::SimPolyEval
            | SyscallNumbers::SimPow
            | SyscallNumbers::SimFoldChain
            | SyscallNumbers::SimConstraintEval
            | SyscallNumbers::SimDomainPoints
            | SyscallNumbers::SimRegisterCommit
            | SyscallNumbers::SimVerifyPathBatch => None,
        }
    }
}
        ...
        Ok(match self {
            ...
            Instruction::EcallEbreak => {
                ...
                match syscall_number {
                    ...
                    SyscallNumbers::SimPolyEval => {
                        // MEASUREMENT-ONLY (never proven). sim/27: evaluate the FRI
                        // terminal polynomial at the queried codeword positions.
                        // a0 = &PolyEvalInput. Carries points/coeffs in the log for
                        // the CLI's projected-row counter.
                        let input_ptr = registers.read(10)?;
                        let points = memory.load_doubleword(input_ptr.wrapping_add(5 * 8))?; // positions_len
                        let coeffs = memory.load_doubleword(input_ptr.wrapping_add(8))?; // coeffs_len
                        sim_poly_eval(memory, input_ptr)?;
                        src2_val = points;
                        dst_val = coeffs;
                    }
                    SyscallNumbers::SimPow => {
                        // MEASUREMENT-ONLY (never proven). sim/27: base^exp over
                        // Goldilocks (a1=1) or Fp3 (a1=3). a0=base_ptr, a2=exponent,
                        // a3=out_ptr. Carries width/exponent in the log for the
                        // CLI's per-kind bit-length counter.
                        let base_ptr = registers.read(10)?;
                        let num_limbs = registers.read(11)?;
                        let exponent = registers.read(12)?;
                        let out_ptr = registers.read(13)?;
                        sim_pow(memory, base_ptr, num_limbs, exponent, out_ptr)?;
                        src2_val = num_limbs;
                        dst_val = exponent;
                    }
                    SyscallNumbers::SimFoldChain => {
                        // MEASUREMENT-ONLY (never proven). sim/27: the whole
                        // per-query FRI fold butterfly chain. a0 = &FoldChainInput.
                        // Carries the layer count in the log for the CLI counter.
                        let input_ptr = registers.read(10)?;
                        let num_layers = memory.load_doubleword(input_ptr.wrapping_add(5 * 8))?; // num_layers
                        sim_fold_chain(memory, input_ptr)?;
                        src2_val = num_layers;
                        dst_val = input_ptr;
                    }
                    SyscallNumbers::SimConstraintEval => {
                        // MEASUREMENT-ONLY (never proven). sim/27 v2: offload one
                        // table's OOD constraint evaluation to the CLI-registered
                        // evaluator (which runs the recursion verifier's constraint
                        // IR interpreter). a0 = &ConstraintEvalInput. Carries
                        // num_constraints + program node count for the CLI counter.
                        let input_ptr = registers.read(10)?;
                        let (num_constraints, node_count) = sim_constraint_eval(memory, input_ptr)?;
                        src2_val = num_constraints;
                        dst_val = node_count;
                    }
                    SyscallNumbers::SimDomainPoints => {
                        // MEASUREMENT-ONLY (never proven). sim/31: batched primary
                        // FRI query evaluation points. a0 = &DomainPointsInput.
                        // Carries iotas_len (points) in the log for the CLI counter.
                        let input_ptr = registers.read(10)?;
                        let points = memory.load_doubleword(input_ptr.wrapping_add(8))?; // iotas_len
                        sim_domain_points(memory, input_ptr)?;
                        src2_val = points;
                        dst_val = input_ptr;
                    }
                    SyscallNumbers::SimRegisterCommit => {
                        // MEASUREMENT-ONLY (never proven). sim/31: REGISTER
                        // preprocessed commitment via the CLI-registered evaluator.
                        // a0 = &RegisterCommitInput. Carries init_len for the CLI
                        // counter.
                        let input_ptr = registers.read(10)?;
                        let init_len = sim_register_commit(memory, input_ptr)?;
                        src2_val = init_len;
                        dst_val = input_ptr;
                    }
                    SyscallNumbers::SimVerifyPathBatch => {
                        // MEASUREMENT-ONLY (never proven). sim/34: verify every
                        // committed FRI layer's Merkle opening for one query in one
                        // call (subsumes per-layer HASH_FELTS + VERIFY_PATH).
                        // a0 = &VerifyPathBatchInput. Carries the layer count in the
                        // log for the CLI counter.
                        let input_ptr = registers.read(10)?;
                        let num_layers = sim_hash::verify_path_batch(memory, input_ptr)?;
                        src2_val = num_layers;
                        dst_val = input_ptr;
                    }
                    SyscallNumbers::FextLoad => {
...
pub enum ExecutionError {
    ...
    FextOperandOverlap,
    #[error("sim-pow stub: element limb count (width) must be 1 or 3: {0}")]
    SimPowBadWidth(u64),
    #[error("sim-poly-eval stub: codeword length must be a non-zero power of two: {0}")]
    SimPolyEvalBadDomain(u64),
    #[error("sim-constraint-eval stub: eval ecall before REGISTER_CONSTRAINT_PROGRAM")]
    SimConstraintNoProgram,
    #[error("sim-constraint-eval stub: malformed constraint program")]
    SimConstraintBadProgram,
}
