...
// `MAX_PRIVATE_INPUT_SIZE` and `PRIVATE_INPUT_START_INDEX` are sourced from
// the workspace-wide `constants` module so the no_std verifier and the
// std-only executor see the same values. `add-recursion` bumped
// MAX_PRIVATE_INPUT_SIZE to 64 MB to fit serialized STARK proofs as
// recursion input.
