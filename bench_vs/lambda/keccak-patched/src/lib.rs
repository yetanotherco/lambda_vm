...
// =====================================================================
// Lambda VM precompile shim: on the riscv64 guest target, route the full
// 24-round Keccak-f[1600] permutation through the `KeccakPermute` syscall
// instead of running 24 rounds of soft Keccak in pure software.
//
// `round_count == 24` is the only case the precompile handles (it always
// performs the standard 24-round Keccak-f). For the unusual case
// `round_count != 24` we still emit a software fallback under a different
// name so the impl_keccak! macro can be reused.
// =====================================================================

#[cfg(target_arch = "riscv64")]
const KECCAK_SYSCALL_NUMBER: usize = usize::MAX - 1;

/// Issue the lambda-vm `KeccakPermute` syscall: full 24 rounds of
/// Keccak-f[1600] applied in-place to the 25-lane u64 state.
#[cfg(target_arch = "riscv64")]
#[inline(always)]
...
// but kept for API completeness so other consumers don't silently break).
#[cfg(target_arch = "riscv64")]
impl_keccak!(p1600_software, f1600_software, u64);

/// Keccak-p[1600, rc] permutation (lambda-vm riscv64 guest).
///
/// For the standard `round_count == 24`, dispatches to the lambda-vm
/// `KeccakPermute` precompile via ecall. Falls back to the upstream
/// pure-Rust implementation for any non-standard round count.
#[cfg(target_arch = "riscv64")]
pub fn p1600(state: &mut [u64; PLEN], round_count: usize) {
    if round_count == 24 {
        keccak_permute_syscall(state);
    } else {
        p1600_software(state, round_count);
    }
}

/// Keccak-f[1600] permutation (lambda-vm riscv64 guest).
#[cfg(target_arch = "riscv64")]
