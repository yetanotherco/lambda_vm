...
#
#   -n "N1 N2 ..."     permutation counts to sweep (default: "1365 5000 5461 21845")
...
#   -h, --help
#
# Runs on the CPU bench box. Expect ~10 minutes for the default sweep.
...
if [ "$SKIP_BUILD" = true ]; then
    ...
    echo -e "${GREEN}Building release CLI (jemalloc-stats)...${NC}"
...
BLOWUP="$BLOWUP" CSV="$CSV" python3 - "$RESULTS" <<'PY'
...
print(f"{os.environ['GIT_BRANCH']} @ {os.environ['GIT_REV']}  |  "
...
if csv:
    ...
    with open(csv, "a") as fh:
        if new:
            fh.write("git_rev,mode,blowup,cores,n_perms,cycles,cells,epochs,"
                     "median_s,min_s,max_s,cv_pct,keccaks_per_s,peak_mb\n")
        for r in rows:
            fh.write(f"{os.environ['GIT_REV']},{os.environ['MODE']},{os.environ['BLOWUP']},"
