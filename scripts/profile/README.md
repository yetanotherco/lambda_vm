...
| `zisk_profile.sh` | Nsight Compute on **ZisK's** hot kernels, the same way run B profiles ours: its LDE (`nttDif/DitColMajorKernel`) and Merkle commit (`linearHashTiledKernel_pos1`, `merkleNode(Warp)Kernel_pos1`, BLAKE3) on one matrix of ZisK's Main shape, built from the published `proofman-starks-src` 1.3.0-alpha crate; then (`ZP_PROVE=1`) the public ZisK 1.3.0-alpha release and Poseidon1 key, one prove of a committed zisk-eth-client workload under ncu for its expression (`gen_basic_*`), DEEP (`computeFRIExpressionFolded`), fold and eval kernels. Why: `thoughts/zf/gap/W3-KERNELS.md`. | yes (`ZP_NO_NCU=1` = plumbing check only) | stage 1 ~10 min; stage 2 first run ~40-90 min (5.2 GB key download, ~30 GB disk) |
