...
# Leaves OUT/ and OUT.tar.gz; read OUT/ncu_summary.txt and OUT/*.details.txt first.
set -euo pipefail
...
PROBE="$(pf_build_probe "$OUT/probe" "$GPU")" || pf_die "could not build the one-kernel probe: see $OUT/probe/pf_probe.build.log"
# the passes' own flags, so a section or option this ncu rejects fails here, before the builds
COUNTERS="$(pf_counter_probe "$NCU_BIN" "$PROBE" "$OUT/probe/pf_probe.ncu.log" "${SECTIONS[@]}" \
  -k 'regex:^pf_probe_kernel$' -c 1 -f -o "$OUT/probe/pf_probe_ncu")"
pf_log "$COUNTERS"
...
build_bench() { # build_bench <tag> <cargo test args...> ; leaves $OUT/cargo-<tag>.json
  local tag="$1" rc=0
  shift
  ...
  env -u RUSTFLAGS -u CARGO_BUILD_RUSTFLAGS -u CARGO_ENCODED_RUSTFLAGS -u LAMBDA_VM_RPX_MAXRREGCOUNT -u LAMBDA_VM_NVCC_LINEINFO \
    ...
    > "$OUT/cargo-$tag.json" 2> "$OUT/build-$tag.log" || rc=$?
  if [ "$rc" -ne 0 ]; then tail -20 "$OUT/build-$tag.log"; pf_die "build failed (rc=$rc): $OUT/build-$tag.log"; fi
}
...
RPX_BIN="$(python3 "$HERE/lib/cargo_test_bin.py" exe --name rpx_occupancy_sweep --kind test < "$OUT/cargo-rpx.json")" || pf_die "no rpx_occupancy_sweep binary"
check_cubins "$OUT/cargo-rpx.json"
build_bench sumcheck -p math-cuda --release --test sumcheck
SC_BIN="$(python3 "$HERE/lib/cargo_test_bin.py" exe --name sumcheck --kind test < "$OUT/cargo-sumcheck.json")" || pf_die "no sumcheck test binary"
check_cubins "$OUT/cargo-sumcheck.json"
...
ncu_pass() { # ncu_pass <name> <kernel regex> <launch count> <binary> <test name>
  ...
  env -i "${PF_ENV[@]}" timeout --signal=INT --kill-after=60 "$STEP_TIMEOUT" "$NCU_BIN" "${SECTIONS[@]}" \
    -k "regex:$regex" -c "$count" -f -o "$OUT/$name" "$bin" "$test" --exact --ignored --nocapture \
    > "$OUT/$name.stdout" 2>&1 || rc=$?
  pf_log "ncu $name: rc=$rc, $(grep -c '^==PROF== Profiling' "$OUT/$name.stdout" || true) launch(es) profiled"
}
...
for r in "$OUT"/*.ncu-rep; do
  ...
  timeout 600 "$NCU_BIN" --import "$r" --page details > "${r%.ncu-rep}.details.txt" 2>&1 || true
  timeout 600 "$NCU_BIN" --import "$r" --page details --csv > "${r%.ncu-rep}.details.csv" 2>/dev/null || true
  timeout 600 "$NCU_BIN" --import "$r" --page raw --csv > "${r%.ncu-rep}.raw.csv" 2>/dev/null || true
done
if compgen -G "$OUT/*.raw.csv" > /dev/null; then
  python3 "$HERE/lib/ncu_summary.py" --out "$OUT" "$OUT"/*.raw.csv > /dev/null 2>&1 || pf_log "ncu_summary.py failed (the .details.txt files stand)"
fi
...
if [ -z "$NSYS_BIN" ]; then
  ...
  PROVER_BIN="$(python3 "$HERE/lib/cargo_test_bin.py" exe --name lambda_vm_prover --kind lib < "$OUT/cargo-prover.json")" || pf_die "no prover test binary"
  ...
  (cd "$REPO/prover" && exec env -i "${PF_ENV[@]}" LAMBDA_VM_BENCH_BACKEND=whir LAMBDA_VM_BENCH_ELF=ethrex \
     ...
     "$NSYS_BIN" profile --trace="cuda,nvtx,osrt" --stats=false -f true -o "$OUT/whir-prover-timeline" \
     "$PROVER_BIN" tests::multilinear_bench_tests::continuations --exact --ignored --nocapture) > "$OUT/nsys.stdout" 2>&1 || rc=$?
  pf_log "nsys: rc=$rc"
  if [ -s "$OUT/whir-prover-timeline.nsys-rep" ]; then
    timeout "$STEP_TIMEOUT" "$NSYS_BIN" export --type sqlite --force-overwrite true --output "$OUT/whir-prover-timeline.sqlite" \
      "$OUT/whir-prover-timeline.nsys-rep" > /dev/null 2>&1 || true
    timeout "$STEP_TIMEOUT" "$NSYS_BIN" stats --report cuda_gpu_kern_sum,cuda_api_sum --format csv \
      --output "$OUT/whir-prover-timeline" "$OUT/whir-prover-timeline.sqlite" > /dev/null 2>&1 || true
    python3 "$HERE/lib/nsys_block_summary.py" --sqlite "$OUT/whir-prover-timeline.sqlite" --log "$OUT/nsys.stdout" \
      --out "$OUT/whir-prover-timeline.summary" --quiet > /dev/null 2>&1 || pf_log "nsys summary failed"
  fi
fi
...
pf_log "DONE: send back $TAR ($(du -h "$TAR" | cut -f1)). Read first: ncu_summary.txt, then *.details.txt"
