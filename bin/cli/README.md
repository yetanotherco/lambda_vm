...
```sh
./target/release/cli execute <PROGRAM.elf> [--private-input <FILE>] [--flamegraph <FILE>]
```
...
```sh
./target/release/cli prove <PROGRAM.elf> -o proof.bin [flags]
```
...
```sh
./target/release/cli verify <PROOF> <PROGRAM.elf> [flags]
```
...
```sh
./target/release/cli count-elements <PROGRAM.elf> [--private-input <FILE>]
```
...
./target/release/cli prove program.elf -o /tmp/proof.bin --private-input input.bin --time --cycles
```
...
```sh
./target/release/cli execute <PROGRAM.elf> --flamegraph folded.txt
```
...
./target/release/cli execute executor/program_artifacts/bench/quicksort.elf --flamegraph /tmp/quicksort.txt
